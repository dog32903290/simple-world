...
REGISTER_SELFTESTS(/*orderBase=*/116,
    ...
    // (--selftest-transformpointsfromclipspace RETIRED 2026-07-10, 廢棄節點退場 — the .t3 compound
    //  provides TransformFromClipSpace now (via a legacy-name alias); kernel math in
    //  --selftest-mathv-transformfromclipspace, takeover in t3-transformfromclipspace-retire.)
    ...
    {"gridplane", runGridPlaneSelfTest},  // GridPlane hand-crafted DrawKind: procedural-grid quad, closed-form center pixel (-bug drops the Seam-2 render-state stamp -> opaque, dimmer)
    ...
    {"t3-depthtolinear", runT3DepthToLinearGates},  // ★TEXTURE-COMPUTE SEAM stage 2 (SRV-tex read + b0 CB): _ComputeDepthToLinear.t3 (guid ade1d03d…) → collapseTextureComputeStageSrv folds SrvFromTexture2d/UavFromTexture2d/FloatsToBuffer/GetTextureSize/CalcInt2Dispatch/SamplerState onto ONE ComputeShaderStageTex → resident cook (t3xf_depth_source fixture) + R32Float readback vs depth-linearization oracle. ①takeover(SRV wire + Near/Far CB0/CB1 LIVE-wired + CB2..CB5 baked) ③reference ②parity(seed Near/Far) ④layout under one -bug
    {"t3-depthtolinear-cblive", runT3DepthToLinearCbLiveGates},  // ★TEXTURE-COMPUTE SEAM stage 2 CB-LIVE (§5 row 2 "留白" closed): collapseTextureComputeStageSrv WIRES boundary Near→CB0/Far→CB1 (fork computestagetex-cb-live-boundary-wire). ⑤liveness: seed two Near/Far via buildEvalGraph → each cook tracks its OWN depth oracle + A≠B (non-vacuous); injectBug=force-bake → seed inert → outB frozen → RED
    {"t3-srvtexfold", runT3SrvTexFoldGates},  // ★TEXTURE-COMPUTE SEAM stage 3 IMPORTER (§5 row 3 "留白" closed): buffer-rail SrvFromTexture2d fold — appendSrvFromTexFold re-anchors SrvFromTexture2d.out→Stage.ShaderResources onto Stage.ShaderResourceTextures. Synthetic .t3 structural gate: ①texFolded ②bufKept; injectBug=t3SrvTexFoldDisable → texture wire lost → RED. Cook proven by --selftest-t3-getimagebrightness
    {"t3-getimagebrightness", runT3GetImageBrightnessGates},  // ★TEXTURE-COMPUTE SEAM stage 3 (BUFFER-rail stage reads a Texture2D SRV): programmatic compound (texfix→ShaderResourceTextures / zerofix→Uavs / scalefix→ConstantBuffers, KernelName=computeshaderstage_getimagebrightness) → resident cook (CROSS-CURRENCY gather + setTexture + default sampler + 2D-from-SRV-tex dispatch) + uint UAV readback vs luminance-sum oracle. ①parity(injectBug=skip texture bind → unbound read → 0) ②reachability. Kernel 逐格 by --selftest-mathv-getimagebrightness
