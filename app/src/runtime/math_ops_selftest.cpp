...
#include "runtime/math_ops_selftest_util.h"  // shared evalOpParams (also used by the _extra leaf)

namespace sw {

// vec2 + logic + misc-vec teeth live in math_ops_selftest_extra.cpp (this file was AT its line cap).
...
// Helper: Remap — five-input op (+ optional BiasAndGain shaping and Mode).
// bgX/bgY default to (0.5,0.5) = identity (no reshaping); mode defaults to 0 (Normal).
// These defaults match the Remap.t3 spec defaults, so existing 5-arg callers are unchanged.
float evalRemapOp(float value, float inMin, float inMax, float outMin, float outMax,
                  float bgX = 0.5f, float bgY = 0.5f, float mode = 0.0f) {
  ...
  g.node(nid)->params["BiasAndGain.x"] = bgX;  // X = gain (evalRemap in[5])
  g.node(nid)->params["BiasAndGain.y"] = bgY;  // Y = bias (evalRemap in[6])
  g.node(nid)->params["Mode"]          = mode; // 0=Normal 1=Clamped 2=Modulo (in[7])
  int outIdx = -1;
  for (size_t i = 0; i < spec->ports.size(); ++i)
    if (spec->ports[i].id == "out") { outIdx = (int)i; break; }
  EvaluationContext ctx{};
...
// Helper: Lerp — three-input op (+ optional Clamp bool).
// clamp defaults to 0.0f (false) = unclamped, matching the Lerp.t3 spec default, so existing
// 3-arg callers are unchanged. clamp>0.5 → f is clamped to [0,1] before the lerp (evalLerp in[3]).
float evalLerpOp(float a, float b, float f, float clamp = 0.0f) {
...
int runMathOpsSelfTest(bool injectBug) {
  ...
  // BiasAndGain ≠ (0.5,0.5): the gain/bias curve reshapes the normalized value before the out-map.
  // TiXL Remap.cs l.28-31: applied only when 0 < normalized < 1. Remap.cs ApplyGainAndBias (MathUtils.cs).
  // value=0.25, in[0,1], out[0,1], gain(bgX)=0.5(identity), bias(bgY)=0.75 → normalized 0.25 reshaped to 0.5.
  //   remapGetBias(0.75,0.25)=0.25/((1/0.75-2)*0.75+1)=0.25/0.5=0.5; schlick gain=0.5 is identity. v=0.5.
  {
    float r = evalRemapOp(0.25f, 0.0f, 1.0f, 0.0f, 1.0f, /*bgX gain*/0.5f, /*bgY bias*/0.75f);
    bool pass = std::fabs(r - 0.5f) < 1e-4f;
    ok = ok && pass;
    printf("[selftest-mathops] Remap(0.25,0,1,0,1,gain=0.5,bias=0.75)=%.4f want=0.5000 (bias reshape) -> %s\n", r, pass ? "PASS" : "FAIL");
  }
  // RED proof: if BiasAndGain were ignored (identity passthrough), the normalized 0.25 would NOT be
  // reshaped → v = 0.25. Assert that no-reshape value (0.25) → FAIL since the real reshaped value is 0.5.
  if (injectBug) {
    float r = evalRemapOp(0.25f, 0.0f, 1.0f, 0.0f, 1.0f, 0.5f, 0.75f);
    bool pass = std::fabs(r - 0.25f) < 1e-4f;  // assert no-reshape → must FAIL (real=0.5)
    ok = ok && pass;
    printf("[selftest-mathops] Remap inject-ignore-bias=%.4f assert=0.25 (INJECT BUG) -> %s\n", r, pass ? "PASS" : "FAIL");
  }
  // Mode=1 (Clamped): out-of-range v is clamped to [min(outMin,outMax), max(...)]. Remap.cs l.40-45.
  // value=1.5, in[0,1], out[0,10] → normalized 1.5 (no reshape, >1), v=15 → Clamped to 10.
  {
    float r = evalRemapOp(1.5f, 0.0f, 1.0f, 0.0f, 10.0f, 0.5f, 0.5f, /*mode Clamped*/1.0f);
    bool pass = std::fabs(r - 10.0f) < eps;
    ok = ok && pass;
    printf("[selftest-mathops] Remap(1.5,0,1,0,10,mode=Clamped)=%.4f want=10.000 -> %s\n", r, pass ? "PASS" : "FAIL");
  }
  // Mode=2 (Modulo): v wrapped by Fmod(v, max-min). Remap.cs l.46-50; MathUtils.Fmod = v-floor(v/d)*d.
  // value=1.5, out[0,10] → v=15, delta=10 → 15 - floor(15/10)*10 = 15-10 = 5.
  {
    float r = evalRemapOp(1.5f, 0.0f, 1.0f, 0.0f, 10.0f, 0.5f, 0.5f, /*mode Modulo*/2.0f);
    bool pass = std::fabs(r - 5.0f) < eps;
    ok = ok && pass;
    printf("[selftest-mathops] Remap(1.5,0,1,0,10,mode=Modulo)=%.4f want=5.0000 -> %s\n", r, pass ? "PASS" : "FAIL");
  }
  // RED proof: if Mode were ignored (always Normal passthrough), both Clamped and Modulo would leak
  // the raw v=15. Assert that Normal value (15) for the Clamped call → FAIL since real Clamped v is 10.
  if (injectBug) {
    float r = evalRemapOp(1.5f, 0.0f, 1.0f, 0.0f, 10.0f, 0.5f, 0.5f, 1.0f);
    bool pass = std::fabs(r - 15.0f) < eps;  // assert Normal-passthrough → must FAIL (real Clamped=10)
    ok = ok && pass;
    printf("[selftest-mathops] Remap inject-ignore-mode(Clamped)=%.4f assert=15 (INJECT BUG) -> %s\n", r, pass ? "PASS" : "FAIL");
  }
  ...
  // Clamp=true, F>1: TiXL Lerp.cs l.19-21 clamps f to [0,1] before the lerp.
  // Lerp(0,10,F=2.0,Clamp=1) → f clamped 2.0→1.0 → result = B = 10.
  {
    float r = evalLerpOp(0.0f, 10.0f, 2.0f, /*clamp*/1.0f);
    bool pass = std::fabs(r - 10.0f) < eps;
    ok = ok && pass;
    printf("[selftest-mathops] Lerp(0,10,2.0,Clamp=1)=%.4f want=10.000 (f clamped to B) -> %s\n", r, pass ? "PASS" : "FAIL");
  }
  // Clamp=true, F<0: f clamped 0 → result = A.
  {
    float r = evalLerpOp(5.0f, 20.0f, -1.0f, /*clamp*/1.0f);
    bool pass = std::fabs(r - 5.0f) < eps;
    ok = ok && pass;
    printf("[selftest-mathops] Lerp(5,20,-1.0,Clamp=1)=%.4f want=5.0000 (f clamped to A) -> %s\n", r, pass ? "PASS" : "FAIL");
  }
  // RED proof: if the Clamp=true branch were a no-op (never clamps f), Lerp(0,10,F=2.0,Clamp=1)
  // would leak unclamped 0+(10-0)*2 = 20. Assert that unclamped value (20) → FAIL since real is 10.
  if (injectBug) {
    float r = evalLerpOp(0.0f, 10.0f, 2.0f, 1.0f);
    bool pass = std::fabs(r - 20.0f) < eps;  // assert never-clamped → must FAIL (real clamped=10)
    ok = ok && pass;
    printf("[selftest-mathops] Lerp inject-drop-clamp=%.4f assert=20 (INJECT BUG) -> %s\n", r, pass ? "PASS" : "FAIL");
  }
  ...
  // evalOpParams: shared free helper (math_ops_selftest_util.h) — builds a 1-node graph, sets
  // named Float params, reads a named out pin. Used here (Sqrt..Sigmoid, InvertFloat) and in the
  // _extra leaf (logic + misc-vec).
