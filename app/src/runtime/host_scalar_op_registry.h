...
struct HostScalarCookCtx {
  ...
  float* output = nullptr;                                      // driver-owned scalar slot (port 0); op writes it
  ...
  // *output only. The FIRST multi-output HOST-SCALAR op (PickStringPart pioneered this on the STRING rail).
