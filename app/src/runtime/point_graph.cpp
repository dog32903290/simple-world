...
void PointGraph::cook(const Graph& g, const EvaluationContext& ctx, const SourceRegistry* reg,
  ...
  // The COMMAND-flow walker (Execute/Switch/Loop/ExecRepeatedly collectors + S3a SetVar/C1 Camera SubGraph
  // scopes + per-op cmd dispatch) was extracted VERBATIM into PointGraph::Impl::cookFlatCommand
  // (point_graph_command_cook.cpp — the DEBT "拆 Command 分支" split). This cookCommand std::function slot
  // stays a THIN forwarding lambda so the closure web is untouched: the terminal Command dispatch, the
  // Camera-op Command-subtree recursion, and cookFlatTexNode's FORK#1 (a tex op's Command input) all keep
  // calling through the slot. The method takes the shared cook-stack state by ref — cookNode (Points
  // gather) / cookMeshInto (Mesh) / cookTexNode (FORK#1 Texture2D) / cookCommand (SELF, Command→Command
  // recursion) / cmdVisiting (cycle guard) / ctxVars (S3a scope) — same minimal-shared-state contract as
  // cookFlatTexNode. The resident twin lives in its own TU (genuine flat/resident graph-access divergence).
  std::function<RenderCommand(int)> cookCommand = [&](int id) -> RenderCommand {
    return p_->cookFlatCommand(g, ctx, reg, nodeParams, ctxVars, cmdVisiting, cookNode, cookMeshInto,
                               cookTexNode, cookCommand, id);
  };
