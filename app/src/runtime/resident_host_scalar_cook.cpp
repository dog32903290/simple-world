...
void cookHostScalarNodes(ResidentEvalGraph& g, const ResidentEvalCtx& ctx) {
  for (ResidentNode& rn : g.nodes) {
    ...
    // Write the scalar onto the resident node's MAIN Float output = port index 0 (the host-scalar layout:
    // output port FIRST, FloatListLength.Length / PickFloatFromList.Selected). A SINGLE-output op emits
    // only this. A MULTI-OUTPUT op (AnalyzeFloatList) ALSO fills scalarOutputs[k] for its extra ports →
    // distributed onto extOut[k] (the channel evalResidentFloat reads), bound-guarded by the extOut size.
    rn.extOut[0] = scalar;
