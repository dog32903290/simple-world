...
// warnCookDepthOnce (decl in point_graph_internal.h): de-anonymised so the extracted resident-command TU
// (point_graph_resident_command_cook.cpp) calls the SAME single g_warnedCookDepth flag — the warn-once
// behaviour is process-global, not per-TU. Verbatim body; only its linkage changed.
void pgdetail::warnCookDepthOnce() {
...
void PointGraph::cookResident(const ResidentEvalGraph& rg, const EvaluationContext& ctx,
  ...
  std::function<RenderCommand(const std::string&, int)> cookCommand =
    ...
    // The COMMAND-flow walker (Execute/Switch/Loop/ExecRepeatedly collectors + S3a SetVar/C1 Camera
    // SubGraph scopes + bypass + per-op cmd dispatch) was extracted VERBATIM into
    // PointGraph::Impl::cookResidentCommand (point_graph_resident_command_cook.cpp — the resident half of
    // the DEBT "拆 Command 分支" split). This slot stays a THIN forwarding lambda so the closure web is
    // untouched (the terminal Command dispatch + cookResidentTexNode FORK#1 + the Camera/bypass recursion
    // keep calling through it). NOT a shared path with the flat twin — resident path/ResidentInput access
    // genuinely diverges. kCookDepthCap rides in as depthCap (the SAME cap cookResidentTexNode takes).
    return p_->cookResidentCommand(rg, ctx, reg, kCookDepthCap, ctxVars, nodeParams, cookNode,
                                   cookResidentMesh, cookTexNode, cookCommand, path, depth);
  };
