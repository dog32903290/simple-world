...
//   (Key-compatible, not file-complete: a TiXL theme file carries all 53 keys; an sw file carries 28.
...
//   ThemeHandling.StoreAllColors). sw themes only the 28 fields its ui::theme::apply() actually
//   routes into the ImGui style + node base/datatype colors + the canvas it renders. The remaining 25
//   are DEFERRED because sw does not yet feed them through the theme — e.g. GridLines (connection-snap
//   guides, no sw consumer) and the Widget*/Status*/MiniMapItems/Gray/TextMuted/*Full families have no
//   sw consumer wired to the theme yet. The canvas Bg fill + background grid (CanvasBackground,
//   CanvasGrid) ARE now themed (sw renders them; previously hardcoded). Widening this set is a future
//   "extend themed field coverage" increment: add the field to ui::theme's kFields/struct AND route it
//   where it's drawn. The default theme remains byte-identical to TiXL across all 28 themed fields
//   (verified by --selftest-theme).
//
...
//   only the Colors dict (the 28 themed UiColors fields above). The schema is forward-compatible: a
