...
#include "runtime/view_camera_active.h"  // runViewCameraOverrideSelfTest (phase-B output-camera override seam)
...
int runFieldToImageOutputSelfTest(bool);   // fieldtoimage_output_golden.cpp (production-path SphereSDF→FieldToImage output golden)
...
namespace sw { int runFieldToImageGoldenSelfTest(bool); }          // field_ops_fieldtoimage_golden.cpp (renderFieldToImage kernel: Mode0 gradient-LUT distance-remap + Mode1 UseColor f.rgb-passthrough teeth)
...
// (MeshVerticesToPoints flat atom + its --selftest-meshverticestopoints RETIRED 2026-07-10, 廢棄節點退場 —
//  the .t3 compound (guid 2467e1ed…) provides it now. Kernel math is mathv-verified (selftests_mathv_
//  meshverticestopoints.cpp dispatches app/shaders/meshverticestopoints.metal), takeover in
//  t3import_meshverticestopoints_retire_golden.cpp. See RETIREMENT_BATTLE_SPEC §5.)
...
namespace sw { int runMidiClipSelfTest(bool); }                  // midiclip_golden.cpp (MidiClip SMF reader + Dict fold — minimal Standard MIDI File → Dict<float>)
namespace sw { int runMidiClipNodeSelfTest(bool); }              // dict_ops_midiclip_golden.cpp (MidiClip NODE cook: Filename→.mid→cookFlatDict, bars→ticks via localFxTime)
...
namespace sw { int runGetPositionSelfTest(bool); }               // point_ops_getposition_golden.cpp (flow.context: transform-scope read vs GetPosition.cs:39-51 closed form, WorldSpace/CameraSpace/ClipSpace via default camera + live Camera scope, value-rail latch read, flat + resident identical; -bug forces the Space matrix to Identity → RED)
namespace sw { int runGpuMeasureSelfTest(bool); }                // point_ops_gpumeasure_golden.cpp (render.analyze SMOKE: GPU-time measure mechanism — real render buffer time recorded into the latch (non-zero + monotone 0.03 smoothing, GpuMeasure.cs:65,74); -bug disables the executor record hook → latch stays 0 → RED)
namespace sw { int runSliceViewPortSelfTest(bool); }             // point_ops_sliceviewport_golden.cpp (render.transform: cell viewport rect + RepeatView clip M11/M22 scale vs SliceViewPort.cs:40-73 closed form + cell RequestedResolution push + a real render lands content INSIDE the cell not outside; -bug skips the viewport stamp → content spills full-target → RED)
...
namespace sw { int runExecuteSiblingsStateSelfTest(bool); }      // point_ops_execute_siblings_state_golden.cpp (★Seam 2 EXECUTE-SIBLINGS state-accumulation: flat Rasterizer/OM/IA siblings into Execute accumulate + stamp each Draw — 4 gates: takeover(==GridPlane oracle)/order(Draw before vs after setter)/both-leg(flat==resident)/multi-draw share; -bug disables accumulation → Draws unstamped → RED)
...
namespace sw { int runT3TransformPointsParity(bool); }               // t3import_transformpoints_golden.cpp (★KEYSTONE首證: real TransformPoints.t3 import→buildEvalGraph→cookResident vs xfprobe oracle — MEASURED GREEN (GPU-compute replay LIVE); reports mapped/unmapped children + dropped wires)
...
namespace sw { int runT3NestedCompoundParity(bool); }                // t3import_nestedcompound_golden.cpp (★COMPOUND-RECURSION keystone: DrawPointsDOF.t3 recurses its TransformPoints child into a nested compound via a resolver — structural (nested atomic==false + §1.3 cross-layer Points wire resolved) + cross-boundary parity vs host-matrix oracle; -bug disables recursion → nested absent → RED)
namespace sw { int runT3LayoutGolden(bool); }                        // t3import_layout_golden.cpp (★.t3ui LAYOUT keystone: real TransformPoints.t3+.t3ui via importT3Symbol(…,layoutResolve) → every child/boundary-pin x,y matches the .t3ui Position constants (not sw's own output); -bug (t3LayoutDisable) reverts every position to 0,0 → RED)
namespace sw { int runT3TimeClipOobRegression(bool); }               // t3import_timeclip_oob_golden.cpp (CRASH regression: parseChildTimeClips heap-OOB on a DirtyFlagTrigger-only Outputs entry lacking OutputData (DrawMesh/FindClosestPointsOnMesh.t3); ① no-crash+clips empty ② full TimeClip values ③ absent SourceRange/LayerIndex conform; -bug (t3ClipOobGuardDisable) reverts optField to bare const operator[] → ASan OOB abort)
namespace sw { int runT3TransformPointsRetireGates(bool); }          // t3import_transformpoints_retire_golden.cpp (★廢棄節點退場 PILOT: retired flat TransformPoints atom → references auto-taken-over by the .t3 compound. ①takeover polarity (name→compound; -bug stand-in atom shadows) ③reference reachability+cookability (-bug drop alias→nullptr) ②parity (delegates) ④layout (delegates))
namespace sw { int runT3CombineBuffersRetireGates(bool); }           // t3import_combinebuffers_retire_golden.cpp (★廢棄節點退場 PILOT #2: retired flat CombineBuffers atom → references auto-taken-over by the .t3 compound (guid 4dd8a618…) — proves the seam is not TransformPoints-specific. ①takeover ③reference ②parity (delegates runT3CombineBuffersParity) ④layout (inline .t3ui constants))
namespace sw { int runT3SnapPointsToGridRetireGates(bool); }         // t3import_snappointstogrid_retire_golden.cpp (★廢棄節點退場: retired flat SnapPointsToGrid atom → .t3 compound (guid bc88304a…). ②parity COOK-DRIVEN: import→buildEvalGraph(骨7 boundary inject)→cookResident vs mathvRefSnapPointsToGrid (generic ComputeShaderStage dispatches computeshaderstage_snaptogrid). ①takeover ③reference ②parity ④layout under one -bug)
namespace sw { int runT3SnapPointsToGridParity(bool); }              // t3import_snappointstogrid_retire_golden.cpp (②parity leg, exposed)
namespace sw { int runT3WrapPointPositionRetireGates(bool); }        // t3import_wrappointposition_retire_golden.cpp (★廢棄節點退場: retired flat WrapPointPosition atom → .t3 compound (guid 0814a593…). ②parity COOK-DRIVEN through the generic ComputeShaderStage IN-PLACE (u0-only, no SRV) dispatch vs mathvRefWrapPointPosition. ①takeover ③reference ②parity ④layout under one -bug)
namespace sw { int runT3WrapPointPositionParity(bool); }             // t3import_wrappointposition_retire_golden.cpp (②parity leg, exposed)
namespace sw { int runT3AddNoiseRetireGates(bool); }                 // t3import_addnoise_retire_golden.cpp (★廢棄節點退場: retired flat AddNoise atom → .t3 compound (guid dd586355…). ②parity COOK-DRIVEN vs mathvRefAddNoise with a TRANSCENDENTAL double fraction gate (simplex noise). ①takeover ③reference ②parity ④layout under one -bug)
namespace sw { int runT3AddNoiseParity(bool); }                      // t3import_addnoise_retire_golden.cpp (②parity leg, exposed)
namespace sw { int runT3ClearSomePointsRetireGates(bool); }         // t3import_clearsomepoints_retire_golden.cpp (★廢棄節點退場: retired flat ClearSomePoints atom → .t3 compound (guid e570b2e6…). ②parity COOK-DRIVEN vs mathvRefClearSomePoints (branchy per-block hash kill, Scale→NAN; b0 float Ratio + b1 int Seed/Repeat/Resolution). ①takeover ③reference ②parity ④layout under one -bug)
namespace sw { int runT3ClearSomePointsParity(bool); }               // t3import_clearsomepoints_retire_golden.cpp (②parity leg, exposed)
namespace sw { int runT3SnapToPointsRetireGates(bool); }             // t3import_snaptopoints_retire_golden.cpp (★廢棄節點退場: retired flat SnapToPoints atom → .t3 compound (guid 5822b0d8…). ②parity COOK-DRIVEN vs mathvRefSnapToPoints (DUAL-SRV index-paired snap, Position+FX1(W); b0 [BlendFactor,Distance,MaxAmount]; stride 32→64 fork). ①takeover ③reference ②parity ④layout under one -bug)
namespace sw { int runT3SnapToPointsParity(bool); }                  // t3import_snaptopoints_retire_golden.cpp (②parity leg, exposed)
namespace sw { int runT3BlendPointsRetireGates(bool); }              // t3import_blendpoints_retire_golden.cpp (★廢棄節點退場: retired flat BlendPoints atom → .t3 compound (guid 2dc5c9d1…). ②parity COOK-DRIVEN vs mathvRefBlendPoints (DUAL-SRV index-paired blend, Position+FX1; b0 [BlendFactor,BlendMode,Pairing,Width,Scatter]; generic-seam per-SRV countB aux extension). ①takeover ③reference ②parity ④layout under one -bug)
namespace sw { int runT3BlendPointsParity(bool); }                   // t3import_blendpoints_retire_golden.cpp (②parity leg, exposed)
namespace sw { int runT3MeshVerticesToPointsRetireGates(bool); }     // t3import_meshverticestopoints_retire_golden.cpp (★廢棄節點退場: retired flat MeshVerticesToPoints atom → .t3 compound (guid 2467e1ed…). ②parity COOK-DRIVEN vs mathvRefMeshVerticesToPoints (Mesh SRV → fresh Point UAV, Stride=64 native, no PbrVertex fork; b0 [OffsetByTBN.x,W]). ①takeover ③reference ②parity ④layout under one -bug)
namespace sw { int runT3MeshVerticesToPointsParity(bool); }          // t3import_meshverticestopoints_retire_golden.cpp (②parity leg, exposed)
namespace sw { int runT3TransformFromClipSpaceRetireGates(bool); }   // t3import_transformfromclipspace_retire_golden.cpp (★廢棄節點退場: retired flat TransformPointsFromClipspace atom → .t3 compound (guid 81377edc…) via graph_bridge.cpp's Pass 3 legacy-name alias (sw name diverges from the .t3's TiXL-canonical "TransformFromClipSpace"). ②parity COOK-DRIVEN vs mathvRefTransformFromClipSpace (camera unproject; b0=TransformsConstBuffer via the empty-FloatsToBuffer-reindex fork). ①takeover ③reference ②parity ④layout under one -bug)
namespace sw { int runT3TransformFromClipSpaceParity(bool); }        // t3import_transformfromclipspace_retire_golden.cpp (②parity leg, exposed)
namespace sw { int runT3ReorientLinePointsRetireGates(bool); }       // t3import_reorientlinepoints_retire_golden.cpp (★廢棄節點退場: retired flat ReorientLinePoints atom → .t3 compound (guid 5dbe204c…), name matches TiXL exactly (no legacy alias). ②parity COOK-DRIVEN vs mathvRefReorientLinePoints (neighbour-tangent align qSlerp; b0 9-float tight array, only cb0[3]=Amount live; copy-through + real OOB-guard forks). ①takeover ③reference ②parity ④layout under one -bug)
namespace sw { int runT3BrdfLookupGates(bool); }                     // t3import_brdflookup_golden.cpp (★TEXTURE-COMPUTE SEAM 首證 stage 1: real _ComputeBRDFLookup.t3 (guid 08d526d0…) COLLAPSES onto ONE tex-track ComputeShaderStageTex atom → resident cook + RGBA16 readback vs split-sum BRDF oracle. ①takeover(collapse fold) ③reference ②parity(UAV-tex write) ④layout under one -bug)
namespace sw { int runT3DepthToLinearGates(bool); }                  // t3import_depthtolinear_golden.cpp (★TEXTURE-COMPUTE SEAM stage 2: SRV-tex read + b0 CB. real _ComputeDepthToLinear.t3 (guid ade1d03d…) → collapseTextureComputeStageSrv folds the SRV/UAV/CB/size/sampler glue onto ONE ComputeShaderStageTex → resident cook (t3xf_depth_source fixture) + R32Float readback vs depth-linearization oracle. ①takeover(SRV wire + Near/Far CB0/CB1 LIVE-wired + CB2..CB5 baked) ③reference ②parity(seed Near/Far) ④layout under one -bug)
namespace sw { int runT3DepthToLinearCbLiveGates(bool); }            // t3import_depthtolinear_cblive_golden.cpp (★TEXTURE-COMPUTE SEAM stage 2 CB-LIVE refinement: collapseTextureComputeStageSrv WIRES Near→CB0/Far→CB1 to the boundary (fork computestagetex-cb-live-boundary-wire) instead of baking. ⑤liveness: seed two Near/Far via buildEvalGraph → each readback tracks its OWN depth-linearization oracle + A≠B (non-vacuous). injectBug=computeShaderStageTexForceBakeCb → CB baked → seed inert → outB frozen → RED)
namespace sw { int runT3SrvTexFoldGates(bool); }                     // t3import_srvtexfold_golden.cpp (★TEXTURE-COMPUTE SEAM stage 3 IMPORTER: buffer-rail SrvFromTexture2d fold. appendSrvFromTexFold re-anchors SrvFromTexture2d.out→Stage.ShaderResources onto Stage.ShaderResourceTextures (§5 row 3 "留白" closed). Synthetic .t3 structural gate: ①texFolded(boundary Texture→ShaderResourceTextures) ②bufKept(buffer-SRV stays on ShaderResources). injectBug=t3SrvTexFoldDisable → texture wire lost → RED. Cook proven by --selftest-t3-getimagebrightness)
namespace sw { int runT3GetImageBrightnessGates(bool); }             // t3import_getimagebrightness_golden.cpp (★TEXTURE-COMPUTE SEAM stage 3: BUFFER-rail ComputeShaderStage reads a Texture2D SRV. Programmatic compound (texfix→ShaderResourceTextures / zerofix→Uavs / scalefix→ConstantBuffers, KernelName=computeshaderstage_getimagebrightness) → resident cook (cross-currency gather + setTexture + default sampler + 2D-from-SRV-tex dispatch) + uint UAV readback vs luminance-sum oracle. ①parity(injectBug skip texture bind → unbound read → 0) ②reachability(ShaderResourceTextures port). Kernel proven 逐格 by --selftest-mathv-getimagebrightness)
