...
namespace sw { int runFieldSubDivPattern3dGoldenSelfTest(bool); } // field_ops_subdivpattern3d_golden.cpp (zero-input color generator; ComputeSubdivision host-oracle, f.r-read template)
...
namespace sw { int runRaymarchPointsSelfTest(bool); }  // raymarchpoints_golden.cpp (SDF point-modify + count-multiply seam, TWO modes: LinePoints -> RaymarchPoints(Field=SphereSDF) -> count=src*(MaxSteps+1)*(clampRefl+1); MODE0 line[1] on-sphere, MODE1 last-step on-sphere; -bug severs Field -> pass-through)
namespace sw { int runPointColorWithFieldSelfTest(bool); }  // pointcolorwithfield_golden.cpp (direct-Field gather LEAF: GridPoints -> SetPointAttributes(seed) -> PointColorWithField(Field=SphereSDF) -> Color recolored white; -bug severs Field -> pass-through seed)
namespace sw { int runSelectPointsWithSdfSelfTest(bool); }  // selectpointswithsdf_golden.cpp (direct-Field gather LEAF: GridPoints -> SelectPointsWithSDF(Field=SphereSDF) -> FX1=max(0,1-2*dist); -bug severs Field -> pass-through FX1=0)
...
namespace sw { int runFindClosestPointsOnMeshSelfTest(bool); }   // point_ops_findclosestpointsonmesh.cpp (★mesh-into-points seam: snap each point onto nearest mesh surface; brute-force tri loop, hand-computed golden + degenerate-tri RED)
...
namespace sw { int runMeshCubeUvGoldenSelfTest(bool); }          // mesh_cube_uv_golden.cpp (mesh generate: CubeMesh non-default UV mappers 1/2/3, closed-form atlas)
...
namespace sw { int runSetPlaybackSelfTest(bool); }              // setplayback_golden.cpp ([SetPlaybackTime]/[SetPlaybackSpeed]: op → PlaybackProvider → g_transport.scrub/setRate)
...
namespace sw { int runSpreadIntoGridSelfTest(bool); }            // point_ops_spreadintogrid_golden.cpp (render lane: per-WIRE grid translation vs SpreadIntoGrid.cs:47-51 closed form, 5 Draw wires cook-through flat + resident byte-identical; -bug collapses every child onto index 0 → RED)
namespace sw { int runSpreadLayoutSelfTest(bool); }              // point_ops_spreadlayout_golden.cpp (render lane: per-WIRE line-spread SRT vs SpreadLayout.cs:34/59-60 closed form + yaw=r.Y mapping + IsEnabled gate, cook-through flat + resident; -bug collapses every child onto index 0 → RED)
namespace sw { int runGetScreenPosSelfTest(bool); }              // point_ops_getscreenpos_golden.cpp (render lane: world→screen projection vs GetScreenPos.cs:22-44 + GraphicsMath closed form, default camera + live Camera scope, value-rail latch read, flat + resident identical; -bug drops the cs:29-33 perspective division → RED)
namespace sw { int runRenderStateBothLegSelfTest(bool); }        // point_ops_renderstate_golden.cpp (★Seam 2 HARNESS-FIRST: Rasterizer→DrawPoints→RenderTarget through flat + resident — stamped FrozenRenderState tuples byte-identical; -bug corrupts flat cull)
namespace sw { int runRasterizerStateSelfTest(bool); }           // point_ops_renderstate_golden.cpp (Seam 2 CLOSED-FORM: cull/fill/winding → exact MTL enum + DX11 defaults; -bug collapses Back→None)
namespace sw { int runBlendStateSelfTest(bool); }                // point_ops_renderstate_golden.cpp (Seam 2 CLOSED-FORM: 7 census blend factors + 3 ops + A2C=false → exact MTL enum; -bug corrupts a factor row)
namespace sw { int runPsoCacheSelfTest(bool); }                  // point_ops_renderstate_golden.cpp (Seam 2: frozenPSOKey — identical tuple=same key, differing FROZEN field=distinct, differing DYNAMIC depthBias=same; -bug constant key)
namespace sw { int runS2GuardsSelfTest(bool); }                  // point_ops_renderstate_golden.cpp (Seam 2 Bucket-C guards: logic-op / dual-source+MRT → port-time reject; valid census tuple must NOT trip; -bug disables guard)
namespace sw { int runS2DepthBiasSelfTest(bool); }               // point_ops_renderstate_golden.cpp (Seam 2 Bucket-B: depthBias -6 pass-through plumbing on-machine; NUMERIC-OUTPUT comparison DEFERRED to Windows-TiXL reference; -bug drops pass-through)
namespace sw { int runOutputMergerBothLegSelfTest(bool); }       // point_ops_outputmerger_golden.cpp (★Seam 2 OutputMerger HARNESS-FIRST: DrawPoints→OutputMerger→RenderTarget through flat + resident — stamped blend/depth FrozenRenderState tuples byte-identical; -bug corrupts flat srcRGB)
namespace sw { int runOutputMergerCookThroughSelfTest(bool); }   // point_ops_outputmerger_golden.cpp (Seam 2 OutputMerger cook-through: real OutputMerger NodeSpec blend/depth params → cookOutputMerger reads → stamped tuple == census combo; -bug strips params → cookParam defaults → RED)
namespace sw { int runInputAssemblerSelfTest(bool); }            // point_ops_inputassembler_golden.cpp (Seam 2 IA CLOSED-FORM: PrimitiveTopology → MTL::PrimitiveType table (D3D_PRIMITIVE_TOPOLOGY → PrimitiveType); -bug corrupts the TriangleList row)
namespace sw { int runInputAssemblerCookThroughSelfTest(bool); } // point_ops_inputassembler_golden.cpp (Seam 2 IA cook-through: real InputAssemblerStage NodeSpec topology=LineList → cookInputAssembler reads → stamped frozen.topology == LineList, both legs byte-identical; -bug strips → cookParam default TriangleList → RED)
namespace sw { int runDrawExplicitSelfTest(bool); }              // point_ops_draw_explicit_golden.cpp (Seam 2 Draw cook-through: real Draw NodeSpec VertexCount=12/Start=4 → cookDrawExplicit → DrawKind::Explicit item with those fields, both legs byte-identical; -bug strips → cookParam defaults 3/0 → RED)
namespace sw { int runDrawExplicitTopologySelfTest(bool); }      // point_ops_draw_explicit_golden.cpp (Seam 2 Draw composition: InputAssembler(LineList) wraps the Draw leaf → the stamped Explicit item's frozen.topology == LineList; -bug strips IA param → default TriangleList → RED)
...
namespace sw { int runGridPointsParitySelfTest(bool); }              // grid_points_parity_golden.cpp (★PARITY-GATE param-completion fan-out #2: GridPoints attribute knobs Color/F1/F2/Orientation/Scale wired vs TiXL .t3/.hlsl — RED-first)
namespace sw { int runLinePointsParitySelfTest(bool); }              // line_points_parity_golden.cpp (★PARITY-GATE param-completion fan-out #3: LinePoints knobs ColorA/ColorB(dual-color lerp)/F1/F2/Orientation/Twist/AddSeparator wired vs TiXL .t3/.hlsl — RED-first)
...
namespace sw { int runDirectionalForceParitySelfTest(bool); }        // directional_force_parity_golden.cpp (★PARITY-GATE Stage-3 Force-class first: DirectionalForce NodeSpec-default vs TiXL .t3 Amount=0.007 cook-through + closed-form kernel-vel — 补验证闸, already-faithful)
namespace sw { int runAxisStepForceParitySelfTest(bool); }           // axisstep_force_parity_golden.cpp (Stage-3 Force fan-out: AxisStepForce CREATE-vel, T2 cook-through 守 NodeSpec SelectRatio=0.1 — 补验证闸)
namespace sw { int runVectorFieldForceParitySelfTest(bool); }        // vectorfield_force_parity_golden.cpp (Stage-3: VectorFieldForce CREATE-vel no-field baked, T2 cook-through 守 NodeSpec — 补验证闸)
namespace sw { int runSnapToAnglesForceParitySelfTest(bool); }       // snaptoangles_force_parity_golden.cpp (Stage-3: SnapToAnglesForce TRANSFORM-vel; cook 種子vel=0→cook-through 只證 no-op 契約, NodeSpec discrim 在 direct-kernel; cook-through-NodeSpec=DEFERRED seam)
namespace sw { int runFieldDistanceForceParitySelfTest(bool); }      // fielddistance_force_parity_golden.cpp (Stage-3: FieldDistanceForce TRANSFORM; baked no-field no-op 契約閘; wired-SDF+cook-through-NodeSpec=DEFERRED)
namespace sw { int runFieldVolumeForceParitySelfTest(bool); }        // fieldvolume_force_parity_golden.cpp (Stage-3: FieldVolumeForce TRANSFORM; baked no-field no-op 契約閘; wired-SDF+cook-through-NodeSpec=DEFERRED)
namespace sw { int runDrawPointsParitySelfTest(bool); }              // drawpoints_parity_golden.cpp (★PARITY-GATE 修偏差: DrawPoints v1 退化4px死點 → DrawKind::Points2 quad-sprite + PointSize/Color cook-through; RED-first PointSize-response + Color-tint 雙牙; -bug 復活4px死點)
namespace sw { int runHexGridPointsParitySelfTest(bool); }           // hexgrid_points_parity_golden.cpp (★PARITY-GATE param-completion fan-out: HexGridPoints 11th input Scale (Size·Scale via .t3 ScaleVector3) wired vs TiXL .hlsl closed-form X; RED-first; -bug re-bakes Scale→1 halving X)
namespace sw { int runMixedMultiInputOrderSelfTest(bool); }          // resident_mixed_multiinput_golden.cpp (骨7b: flatten loop2/loop3 merged into one pass — a MultiInput slot fed by BOTH child + boundary wires keeps wire-declaration order in extraConns; -bug expects old grouped [child…,boundary…] order → RED. Unblocks ~224 mesh-family compounds' marshaled buffer)
namespace sw { int runT3TransformPointsParity(bool); }               // t3import_transformpoints_golden.cpp (★KEYSTONE首證: real TransformPoints.t3 import→buildEvalGraph→cookResident vs xfprobe oracle — MEASURED RED = replay seam hole (no ComputeShaderStage atom); reports mapped/unmapped children + dropped wires)
namespace sw { int runT3CombineBuffersParity(bool); }                // t3import_combinebuffers_golden.cpp (187 量產第一波: real CombineBuffers.t3 code-op replay → concat oracle parity; verifies the replay 量產配方 on an arbitrary compound)
namespace sw { int runT3HseParity(bool); }                           // t3import_hse_golden.cpp (★IMAGE-fx collapse seam: real HSE.t3 (single _multiImageFxSetupStatic wrapper) collapses to sw HSE tex atom → resident tex cook + readback vs hue-shift oracle GREEN; -bug drops FxTexture → RED)
namespace sw { int runT3BlendParity(bool); }                         // t3import_blend_golden.cpp (★IMAGE-fx collapse GENERALIZES: MULTI-child Blend.t3 (6 helper value ops + fx-setup) through the SAME data-driven collapse → Normal-blend oracle (128,128,0,255); -bug drops ImageB → RED. Proves the collapse table is not HSE-only)
namespace sw { int runT3BubbleZoomParity(bool); }                    // t3import_bubblezoom_golden.cpp (★IMAGE-fx collapse GRADIENT-FED: real BubbleZoom.t3 collapses with the GradientsToTexture child ELIDED onto the sw BubbleZoom atom's Gradient port (2×Vector2Components kept) → resident tex cook + readback vs closed-form gradient oracle GREEN; -bug drops the Gradient wire → transparent fallback → RED. Unlocks gradient-fed image-fx)
namespace sw { int runT3RadialGradientParity(bool); }                // t3import_radialgradient_golden.cpp (GRADIENT-FED image GENERATOR: real RadialGradient.t3 collapses with GradientsToTexture ELIDED onto the atom's Gradient port (3×Vector2Components + 3×BoolToFloat + IntToFloat kept) → resident tex cook vs closed-form radial oracle GREEN; -bug drops the Gradient wire → white→black fallback → RED)
namespace sw { int runT3ImportOutputDefsSelfTest(bool); }            // t3import_outputdefs_selftest.cpp (CATALOG-NODE metadata承重: imported RadialGradient.t3 carries sym.outputDefs (type==terminal atom OUTPUT, Texture2D) + real sym.name (from the Id /*RadialGradient*/ comment, not the GUID) → draggable node with an output PIN + non-black view; -bug clears them → dead node → BITE)
namespace sw { int runT3NGonGradientParity(bool); }                  // t3import_ngongradient_golden.cpp (GRADIENT-FED image GENERATOR: real NGonGradient.t3 collapses with GradientsToTexture ELIDED onto the atom's Gradient port (2×Vector2Components + 2×BoolToFloat + IntToFloat kept, 8 boundary FloatParams scalars) → resident tex cook vs closed-form NGon oracle GREEN; -bug drops the Gradient wire → white→black fallback → RED)
namespace sw { int runT3BoxGradientParity(bool); }                   // t3import_boxgradient_golden.cpp (GRADIENT-FED image GENERATOR: real BoxGradient.t3 collapses with GradientsToTexture ELIDED onto the atom's Gradient port (3×Vector2Components + Vector4Components(CornersRadius) + 2×BoolToFloat + IntToFloat kept) → resident tex cook vs closed-form box-SDF oracle GREEN; -bug drops the Gradient wire → white→black fallback → RED)
namespace sw { int runT3RemapColorParity(bool); }                    // t3import_remapcolor_golden.cpp (GRADIENT-FED COLOR + TRANSFORMIMAGE-PASSTHROUGH proof: real RemapColor.t3 collapses with GradientsToTexture + an interposed identity TransformImage + a bypassed GenerateMips ALL ELIDED onto the atom's Gradient port (IntToFloat + BoolToFloat + Vector2Components kept) → resident tex cook of a two-region CheckerBoard input vs sw ColorRemap rowLinear oracle GREEN; -bug drops the Gradient wire → near-black→white gray fallback → RED)
namespace sw { int runT3LinearGradientParity(bool); }                // t3import_lineargradient_golden.cpp (GRADIENT-FED generator + OFFSET-ROUTING-SUBGRAPH: real LinearGradient.t3 collapses with GradientsToTexture ELIDED onto the atom's Gradient port (2×Vector2Components + 2×BoolToFloat + 2×IntToFloat + Multiply + PickFloat kept — the Offset-routing subgraph, fork offset-double-routed-benign-at-default) → resident tex cook vs closed-form linear-ramp oracle GREEN; -bug drops the Gradient wire → black→white fallback → RED)
