/**
 * @file
 * my.numbers.vec2.perlinNoise2 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_PerlinNoise2
 * - Category: Operators/Lib/numbers/vec2
 * - Source: external/tixl/Operators/Lib/numbers/vec2/PerlinNoise2.cs
 * - Primary output(s): VuoPoint2d Result (ColorForValues #868C8D)
 *
 * bounded vector noise adapter. ProofValue is a Vuo-only numeric tap for the Batch 49 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_PerlinNoise2",
					 "description" : "TiXL PerlinNoise2 bounded vector noise adapter. Source: external/tixl/Operators/Lib/numbers/vec2/PerlinNoise2.cs. Category: Operators/Lib/numbers/vec2. Primary output(s): VuoPoint2d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint2d_make(noise2(input1.x * scale, input1.y * scale, seed), noise2(input1.y * scale + 19.17, input1.x * scale - 3.11, seed + 31)); proof = length2(*result);
	*proofValue = proof;
}
