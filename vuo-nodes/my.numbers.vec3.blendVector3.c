/**
 * @file
 * my.numbers.vec3.blendVector3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_BlendVector3
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/BlendVector3.cs
 * - Primary output(s): VuoPoint3d Result (ColorForValues #868C8D)
 *
 * bounded vector blend adapter. ProofValue is a Vuo-only numeric tap for the Batch 51 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_BlendVector3",
					 "description" : "TiXL BlendVector3 bounded vector blend adapter. Source: external/tixl/Operators/Lib/numbers/vec3/BlendVector3.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = mix3(vectorA, vectorB, clampReal(f, 0.0, 1.0)); proof = length3(*result);
	*proofValue = proof;
}
