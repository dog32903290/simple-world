/**
 * @file
 * my.numbers.vec2.dampVec2 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_DampVec2
 * - Category: Operators/Lib/numbers/vec2
 * - Source: external/tixl/Operators/Lib/numbers/vec2/DampVec2.cs
 * - Primary output(s): VuoPoint2d Result (ColorForValues #868C8D)
 *
 * bounded damping adapter. ProofValue is a Vuo-only numeric tap for the Batch 49 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_DampVec2",
					 "description" : "TiXL DampVec2 bounded damping adapter. Source: external/tixl/Operators/Lib/numbers/vec2/DampVec2.cs. Category: Operators/Lib/numbers/vec2. Primary output(s): VuoPoint2d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = mix2(input1, input2, clampReal(damping, 0.0, 1.0)); proof = length2(*result);
	*proofValue = proof;
}
