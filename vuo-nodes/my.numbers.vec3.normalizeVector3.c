/**
 * @file
 * my.numbers.vec3.normalizeVector3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_NormalizeVector3
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/NormalizeVector3.cs
...
VuoModuleMetadata({
					 "title" : "my_NormalizeVector3",
					 "description" : "TiXL NormalizeVector3 exact vector adapter. Source: external/tixl/Operators/Lib/numbers/vec3/NormalizeVector3.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = normalize3(vectorA, factor); proof = length3(*result);
	*proofValue = proof;
}
