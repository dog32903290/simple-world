/**
 * @file
 * my.numbers.float.random.floatHash node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_FloatHash
 * - Category: Operators/Lib/numbers/float/random
 * - Source: external/tixl/Operators/Lib/numbers/float/random/FloatHash.cs
 * - Primary output(s): VuoInteger Result (ColorForValues #868C8D)
 *
 * deterministic hash adapter. ProofValue is a Vuo-only numeric tap for the Batch 48 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_FloatHash",
					 "description" : "TiXL FloatHash deterministic hash adapter. Source: external/tixl/Operators/Lib/numbers/float/random/FloatHash.cs. Category: Operators/Lib/numbers/float/random. Primary output(s): VuoInteger Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
					 "keywords" : [ "tixl", "numbers", "ColorForValues", "#868C8D", "batch48" ],
					 "version" : "1.0.0",
					 "dependencies" : [ ],
				 });
...
)
{
	...
	*result = (VuoInteger)(hashUInt((uint32_t)floor(seedReal * 1000.0 + 0.5), uniqueForChild ? 0x9e3779b9u : 0u) & 0x7fffffff); proof = (VuoReal)(*result % 1000) / 1000.0;
	*proofValue = proof;
}
