/**
 * @file
 * my.image.fx.distort.edgeRepeat node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_EdgeRepeat
 * - Category: Operators/Lib/image/fx/distort
 * - Source: external/tixl/Operators/Lib/image/fx/distort/EdgeRepeat.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/distort/EdgeRepeat.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `EdgeRepeat.hlsl`.
...
VuoModuleMetadata({
 "title":"my_EdgeRepeat",
 "description":"TiXL EdgeRepeat bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/distort/EdgeRepeat.cs. Category: Operators/Lib/image/fx/distort. Primary output evidence: TextureOutput. Shader/resource evidence: EdgeRepeat.hlsl.",
 "keywords":["tixl","image","Texture2D","EdgeRepeat","EdgeRepeat.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_EdgeRepeat Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
