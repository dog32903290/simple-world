/**
 * @file
 * my.image.fx.stylize.fakeLight node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_FakeLight
 * - Category: Operators/Lib/image/fx/stylize
 * - Source: external/tixl/Operators/Lib/image/fx/stylize/FakeLight.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/stylize/FakeLight.t3
 * - Primary output evidence: Output; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `FakeLight.hlsl`.
...
VuoModuleMetadata({
 "title":"my_FakeLight",
 "description":"TiXL FakeLight bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/stylize/FakeLight.cs. Category: Operators/Lib/image/fx/stylize. Primary output evidence: Output. Shader/resource evidence: FakeLight.hlsl.",
 "keywords":["tixl","image","Texture2D","FakeLight","FakeLight.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_FakeLight Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
...
(
		...
		VuoInputData(VuoImage, {"name":"HeightMap"}) heightMap,
