/**
 * @file
 * my.numbers.vec3.vector3Components node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Vector3Components
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/Vector3Components.cs
 * - Primary output(s): VuoReal X, VuoReal Y, VuoReal Z (ColorForValues #868C8D)
 *
 * exact vector adapter. ProofValue is a Vuo-only numeric tap for the Batch 51 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_Vector3Components",
					 "description" : "TiXL Vector3Components exact vector adapter. Source: external/tixl/Operators/Lib/numbers/vec3/Vector3Components.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoReal X, VuoReal Y, VuoReal Z (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*x = vectorA.x; *y = vectorA.y; *z = vectorA.z; proof = fabs(*x) + fabs(*y) + fabs(*z);
	*proofValue = proof;
}
