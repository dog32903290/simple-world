/**
 * @file
 * my.numbers.vec2.process.easeVec2 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_EaseVec2
 * - Category: Operators/Lib/numbers/vec2/process
 * - Source: external/tixl/Operators/Lib/numbers/vec2/process/EaseVec2.cs
 * - Primary output(s): VuoPoint2d Result (ColorForValues #868C8D)
 *
 * bounded vector easing adapter. ProofValue is a Vuo-only numeric tap for the Batch 50 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_EaseVec2",
					 "description" : "TiXL EaseVec2 bounded vector easing adapter. Source: external/tixl/Operators/Lib/numbers/vec2/process/EaseVec2.cs. Category: Operators/Lib/numbers/vec2/process. Primary output(s): VuoPoint2d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	VuoPoint2d target = VuoPoint2d_make(input2d.y, input2d.x); *result = mix2(input2d, target, ease01(clampReal(timeValue / fmax(0.0001, duration), 0.0, 1.0), interpolation)); proof = length2(*result);
	*proofValue = proof;
}
