/**
 * @file
 * my.image.fx.stylize.pixelate node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Pixelate
 * - Category: Operators/Lib/image/fx/stylize
 * - Source: external/tixl/Operators/Lib/image/fx/stylize/Pixelate.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/stylize/Pixelate.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `Pixelate.hlsl`.
...
VuoModuleMetadata({
 "title":"my_Pixelate",
 "description":"TiXL Pixelate bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/stylize/Pixelate.cs. Category: Operators/Lib/image/fx/stylize. Primary output evidence: TextureOutput. Shader/resource evidence: Pixelate.hlsl.",
 "keywords":["tixl","image","Texture2D","Pixelate","Pixelate.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_Pixelate Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
