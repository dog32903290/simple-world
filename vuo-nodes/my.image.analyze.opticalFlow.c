/**
 * @file
 * my.image.analyze.opticalFlow node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_OpticalFlow
 * - Category: Operators/Lib/image/analyze
 * - Source: external/tixl/Operators/Lib/image/analyze/OpticalFlow.cs
 * - Default evidence: external/tixl/Operators/Lib/image/analyze/OpticalFlow.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `OpticalFlowKanade.hlsl`.
...
VuoModuleMetadata({
 "title":"my_OpticalFlow",
 "description":"TiXL OpticalFlow bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/analyze/OpticalFlow.cs. Category: Operators/Lib/image/analyze. Primary output evidence: TextureOutput. Shader/resource evidence: OpticalFlowKanade.hlsl.",
 "keywords":["tixl","image","Texture2D","OpticalFlow","OpticalFlowKanade.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_OpticalFlow Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
