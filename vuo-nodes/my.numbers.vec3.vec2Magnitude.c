/**
 * @file
 * my.numbers.vec3.vec2Magnitude node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Vec2Magnitude
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/Vec2Magnitude.cs
...
VuoModuleMetadata({
					 "title" : "my_Vec2Magnitude",
					 "description" : "TiXL Vec2Magnitude exact vector adapter. Source: external/tixl/Operators/Lib/numbers/vec3/Vec2Magnitude.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoReal Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = length2(input2d); proof = *result;
	*proofValue = proof;
}
