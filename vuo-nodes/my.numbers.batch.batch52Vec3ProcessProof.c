/**
 * @file
 * my.numbers.batch.batch52Vec3ProcessProof node implementation.
 *
 * Proof-only image adapter for Batch 52 Lib.numbers.vec3.process.
 */

#include "VuoImageRenderer.h"
#include <stdlib.h>

VuoModuleMetadata({"title":"my_Batch52Vec3ProcessProof","description":"Proof-only compositor for Batch 52 Lib.numbers.vec3.process.","keywords":["tixl","batch52","numbers","proof"],"version":"1.0.0","dependencies":["VuoImageRenderer"]});

static const char * fragmentShaderSource = VUOSHADER_GLSL_SOURCE(120,
	uniform float easeVec3Value;
	uniform float easeVec3KeysValue;
	uniform float springVec3Value;
	...
	void main()
	{
		...
		float value = 0.0;
		if (row == 0.0) value = easeVec3Value;
		if (row == 1.0) value = easeVec3KeysValue;
		if (row == 2.0) value = springVec3Value;
...
struct nodeInstanceData * nodeInstanceInit(void)
{
	...
	VuoRegister(i, free);
	i->shader = VuoShader_make("my_Batch52Vec3ProcessProof Shader");
...
(
		...
		VuoInputData(VuoReal, {"default":0.1}) easeVec3Value,
		VuoInputData(VuoReal, {"default":0.2}) easeVec3KeysValue,
		VuoInputData(VuoReal, {"default":0.3}) springVec3Value,
...
{
	(void)renderTick;
	VuoShader_setUniform_VuoReal((*instance)->shader, "easeVec3Value", easeVec3Value);
	VuoShader_setUniform_VuoReal((*instance)->shader, "easeVec3KeysValue", easeVec3KeysValue);
	VuoShader_setUniform_VuoReal((*instance)->shader, "springVec3Value", springVec3Value);
