/**
 * @file
 * my.image.fx.stylize.asciiRender node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_AsciiRender
 * - Category: Operators/Lib/image/fx/stylize
 * - Source: external/tixl/Operators/Lib/image/fx/stylize/AsciiRender.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/stylize/AsciiRender.t3
 * - Primary output evidence: Output; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `AsciiRender.hlsl`.
...
VuoModuleMetadata({
 "title":"my_AsciiRender",
 "description":"TiXL AsciiRender bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/stylize/AsciiRender.cs. Category: Operators/Lib/image/fx/stylize. Primary output evidence: Output. Shader/resource evidence: AsciiRender.hlsl.",
 "keywords":["tixl","image","Texture2D","AsciiRender","AsciiRender.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_AsciiRender Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
