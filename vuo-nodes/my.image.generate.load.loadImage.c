/**
 * @file
 * my.image.generate.load.loadImage node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_LoadImage
 * - Category: Operators/Lib/image/generate/load
 * - Source: external/tixl/Operators/Lib/image/generate/load/LoadImage.cs
 * - Default evidence: external/tixl/Operators/Lib/image/generate/load/LoadImage.t3
 * - Primary output evidence: Texture; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `LoadImage.cs`.
...
VuoModuleMetadata({
 "title":"my_LoadImage",
 "description":"TiXL LoadImage bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/generate/load/LoadImage.cs. Category: Operators/Lib/image/generate/load. Primary output evidence: Texture. Shader/resource evidence: LoadImage.cs.",
 "keywords":["tixl","image","Texture2D","LoadImage","LoadImage.cs","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_LoadImage Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
