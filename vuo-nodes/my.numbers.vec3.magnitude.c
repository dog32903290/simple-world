/**
 * @file
 * my.numbers.vec3.magnitude node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Magnitude
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/Magnitude.cs
...
VuoModuleMetadata({
					 "title" : "my_Magnitude",
					 "description" : "TiXL Magnitude exact vector adapter. Source: external/tixl/Operators/Lib/numbers/vec3/Magnitude.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoReal Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = length3(vectorA); proof = *result;
	*proofValue = proof;
}
