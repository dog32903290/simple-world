/**
 * @file
 * my.image.fx.stylize.lightRaysFx node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_LightRaysFx
 * - Category: Operators/Lib/image/fx/stylize
 * - Source: external/tixl/Operators/Lib/image/fx/stylize/LightRaysFx.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/stylize/LightRaysFx.t3
 * - Primary output evidence: Output; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `LightRayFx.hlsl`.
...
VuoModuleMetadata({
 "title":"my_LightRaysFx",
 "description":"TiXL LightRaysFx bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/stylize/LightRaysFx.cs. Category: Operators/Lib/image/fx/stylize. Primary output evidence: Output. Shader/resource evidence: LightRayFx.hlsl.",
 "keywords":["tixl","image","Texture2D","LightRaysFx","LightRayFx.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_LightRaysFx Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
