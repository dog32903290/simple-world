/**
 * @file
 * my.numbers.vec3.vector3Gizmo node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Vector3Gizmo
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/Vector3Gizmo.cs
 * - Primary output(s): VuoPoint3d Result (ColorForValues #868C8D)
 *
 * D gizmo side-effect bounded adapter. ProofValue is a Vuo-only numeric tap for the Batch 51 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_Vector3Gizmo",
					 "description" : "TiXL Vector3Gizmo D gizmo side-effect bounded adapter. Source: external/tixl/Operators/Lib/numbers/vec3/Vector3Gizmo.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = showGizmo ? vectorA : VuoPoint3d_make(0.0, 0.0, 0.0); proof = length3(*result);
	*proofValue = proof;
}
