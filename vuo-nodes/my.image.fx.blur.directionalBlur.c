/**
 * @file
 * my.image.fx.blur.directionalBlur node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_DirectionalBlur
 * - Category: Operators/Lib/image/fx/blur
 * - Source: external/tixl/Operators/Lib/image/fx/blur/DirectionalBlur.cs
 * - Default: Image=null, Size=1, Samples=8, Angle=0, FxTextures=null, FxAngleFactor=1, FxSizeFactor=0, RefinementPass=false, RefinementSamples=6, RefineSizeFactor=0, Wrap=MirrorOnce, Resolution=(0,0) from DirectionalBlur.t3
 * - Primary output: Texture2D TextureOutput (ColorForTextures #9F008A)
 *
 * Bounded Vuo shader adapter for TiXL blur shader evidence including DirectionalBlur.hlsl.
...
VuoModuleMetadata({
					 "title" : "my_DirectionalBlur",
					 "description" : "TiXL DirectionalBlur bounded Vuo shader adapter. Source: external/tixl/Operators/Lib/image/fx/blur/DirectionalBlur.cs. Category: Operators/Lib/image/fx/blur. Primary output: Texture2D TextureOutput (ColorForTextures #9F008A). Default: Image=null, Size=1, Samples=8, Angle=0, FxTextures=null, FxAngleFactor=1, FxSizeFactor=0, RefinementPass=false, RefinementSamples=6, RefineSizeFactor=0, Wrap=MirrorOnce, Resolution=(0,0).",
					 "keywords" : [ "tixl", "texture2d", "image", "fx", "blur", "DirectionalBlur", "DirectionalBlur.hlsl", "bounded approximation", "ColorForTextures", "#9F008A" ],
					 "version" : "1.0.0",
...
struct nodeInstanceData * nodeInstanceInit(void)
{
	...
	instance->shader = VuoShader_make("my_DirectionalBlur Shader");
