/**
 * @file
 * my.numbers.anim.time.dateTimeInSecs node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_DateTimeInSecs
 * - Category: Operators/Lib/numbers/anim/time
 * - Source: external/tixl/Operators/Lib/numbers/anim/time/DateTimeInSecs.cs
 * - Primary output(s): VuoInteger Result (ColorForValues #868C8D)
 *
 * bounded wall-clock adapter. ProofValue is a Vuo-only numeric tap for the Batch 45 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_DateTimeInSecs",
					 "description" : "TiXL DateTimeInSecs bounded wall-clock adapter. Source: external/tixl/Operators/Lib/numbers/anim/time/DateTimeInSecs.cs. Category: Operators/Lib/numbers/anim/time. Primary output(s): VuoInteger Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = (VuoInteger)floor(fmax(0.0, timeValue) + 0.5); proof = (VuoReal)*result;
	*proofValue = proof;
}
