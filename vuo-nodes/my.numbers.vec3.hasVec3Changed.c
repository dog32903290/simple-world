/**
 * @file
 * my.numbers.vec3.hasVec3Changed node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_HasVec3Changed
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/HasVec3Changed.cs
 * - Primary output(s): VuoPoint3d Delta, VuoPoint3d DeltaOnHit, VuoBoolean HasChanged (ColorForValues #868C8D)
 *
 * bounded stateless change adapter. ProofValue is a Vuo-only numeric tap for the Batch 51 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_HasVec3Changed",
					 "description" : "TiXL HasVec3Changed bounded stateless change adapter. Source: external/tixl/Operators/Lib/numbers/vec3/HasVec3Changed.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Delta, VuoPoint3d DeltaOnHit, VuoBoolean HasChanged (ColorForValues #868C8D). ProofValue is Vuo-only.",
					 "keywords" : [ "tixl", "numbers", "ColorForValues", "#868C8D", "batch51" ],
					 "version" : "1.0.0",
					 "dependencies" : [ ],
				 });
...
(
		...
		VuoOutputData(VuoPoint3d, {"name":"Delta"}) delta,
		VuoOutputData(VuoPoint3d, {"name":"DeltaOnHit"}) deltaOnHit,
		VuoOutputData(VuoBoolean, {"name":"HasChanged"}) hasChanged,
...
)
{
	...
	*delta = sub3(vectorA, vectorB); *hasChanged = length3(*delta) > threshold; *deltaOnHit = *hasChanged ? *delta : VuoPoint3d_make(0.0, 0.0, 0.0); proof = *hasChanged ? length3(*delta) : 0.0;
	*proofValue = proof;
}
