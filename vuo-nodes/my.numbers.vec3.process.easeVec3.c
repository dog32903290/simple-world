/**
 * @file
 * my.numbers.vec3.process.easeVec3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_EaseVec3
 * - Category: Operators/Lib/numbers/vec3/process
 * - Source: external/tixl/Operators/Lib/numbers/vec3/process/EaseVec3.cs
 * - Primary output(s): VuoPoint3d Result (ColorForValues #868C8D)
 *
 * bounded vector easing adapter. ProofValue is a Vuo-only numeric tap for the Batch 52 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_EaseVec3",
					 "description" : "TiXL EaseVec3 bounded vector easing adapter. Source: external/tixl/Operators/Lib/numbers/vec3/process/EaseVec3.cs. Category: Operators/Lib/numbers/vec3/process. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = mix3(vectorB, vectorA, ease01(clampReal(timeValue / fmax(0.0001, duration), 0.0, 1.0), interpolation)); proof = length3(*result);
	*proofValue = proof;
}
