/**
 * @file
 * my.numbers.anim.animators.animFloatList node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_AnimFloatList
 * - Category: Operators/Lib/numbers/anim/animators
 * - Source: external/tixl/Operators/Lib/numbers/anim/animators/AnimFloatList.cs
 * - Primary output(s): VuoReal Result (ColorForValues #868C8D)
 *
 * bounded list summary adapter. ProofValue is a Vuo-only numeric tap for the Batch 54 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_AnimFloatList",
					 "description" : "TiXL AnimFloatList bounded list summary adapter. Source: external/tixl/Operators/Lib/numbers/anim/animators/AnimFloatList.cs. Category: Operators/Lib/numbers/anim/animators. Primary output(s): VuoReal Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = bias + amplitude * sin((timeValue + phase + offset) * rate * 6.2831853); proof = *result;
	*proofValue = proof;
}
