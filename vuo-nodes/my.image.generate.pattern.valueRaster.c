/**
 * @file
 * my.image.generate.pattern.valueRaster node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_ValueRaster
 * - Category: Operators/Lib/image/generate/pattern
 * - Source: external/tixl/Operators/Lib/image/generate/pattern/ValueRaster.cs
 * - Default: Image=null, Color=(1,1,1,0.695), Background=(0,0,0,0), MixOriginal=1, Resolution=(0,0), RangeX=(0,1), RangeY=(0,1), MajorLineWidth=1, MinorLineWidth=0.25, Density=(1000,1000) from ValueRaster.t3
 * - Primary output: Texture2D TextureOutput (ColorForTextures #9F008A)
 *
 * Bounded Vuo shader adapter for external/tixl/Operators/Lib/Assets/shaders/img/{fx or generate}/ValueRaster.hlsl.
...
VuoModuleMetadata({
					 "title" : "my_ValueRaster",
					 "description" : "TiXL ValueRaster bounded Vuo shader adapter. Source: external/tixl/Operators/Lib/image/generate/pattern/ValueRaster.cs. Category: Operators/Lib/image/generate/pattern. Primary output: Texture2D TextureOutput (ColorForTextures #9F008A). Default: Image=null, Color=(1,1,1,0.695), Background=(0,0,0,0), MixOriginal=1, Resolution=(0,0), RangeX=(0,1), RangeY=(0,1), MajorLineWidth=1, MinorLineWidth=0.25, Density=(1000,1000).",
					 "keywords" : [ "tixl", "texture2d", "image", "generate", "pattern", "ValueRaster", "ValueRaster.hlsl", "bounded approximation", "ColorForTextures", "#9F008A" ],
					 "version" : "1.0.0",
...
struct nodeInstanceData * nodeInstanceInit(void)
{
	...
	instance->shader = VuoShader_make("my_ValueRaster Shader");
...
)
{
	...
	VuoShader_setUniform_VuoInteger((*instance)->shader, "patternKind", 7);
