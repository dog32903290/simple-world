/**
 * @file
 * my.image.fx.stylize.steps node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Steps
 * - Category: Operators/Lib/image/fx/stylize
 * - Source: external/tixl/Operators/Lib/image/fx/stylize/Steps.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/stylize/Steps.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `Steps.hlsl`.
...
VuoModuleMetadata({
 "title":"my_Steps",
 "description":"TiXL Steps bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/stylize/Steps.cs. Category: Operators/Lib/image/fx/stylize. Primary output evidence: TextureOutput. Shader/resource evidence: Steps.hlsl.",
 "keywords":["tixl","image","Texture2D","Steps","Steps.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_Steps Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
