/**
 * @file
 * my.image.fx.blur.fastBlur node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_FastBlur
 * - Category: Operators/Lib/image/fx/blur
 * - Source: external/tixl/Operators/Lib/image/fx/blur/FastBlur.cs
 * - Default: Image=null, MaxLevels=5 from FastBlur.t3
 * - Primary output: Texture2D Result (ColorForTextures #9F008A)
 *
 * Bounded Vuo shader adapter for TiXL blur shader evidence including FastBlur-BlurPS.hlsl.
...
VuoModuleMetadata({
					 "title" : "my_FastBlur",
					 "description" : "TiXL FastBlur bounded Vuo shader adapter. Source: external/tixl/Operators/Lib/image/fx/blur/FastBlur.cs. Category: Operators/Lib/image/fx/blur. Primary output: Texture2D Result (ColorForTextures #9F008A). Default: Image=null, MaxLevels=5.",
					 "keywords" : [ "tixl", "texture2d", "image", "fx", "blur", "FastBlur", "FastBlur-BlurPS.hlsl", "bounded approximation", "ColorForTextures", "#9F008A" ],
					 "version" : "1.0.0",
...
struct nodeInstanceData * nodeInstanceInit(void)
{
	...
	instance->shader = VuoShader_make("my_FastBlur Shader");
