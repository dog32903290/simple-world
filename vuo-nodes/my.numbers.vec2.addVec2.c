/**
 * @file
 * my.numbers.vec2.addVec2 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_AddVec2
 * - Category: Operators/Lib/numbers/vec2
 * - Source: external/tixl/Operators/Lib/numbers/vec2/AddVec2.cs
...
VuoModuleMetadata({
					 "title" : "my_AddVec2",
					 "description" : "TiXL AddVec2 exact vector adapter. Source: external/tixl/Operators/Lib/numbers/vec2/AddVec2.cs. Category: Operators/Lib/numbers/vec2. Primary output(s): VuoPoint2d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint2d_make(input1.x + input2.x, input1.y + input2.y); proof = length2(*result);
	*proofValue = proof;
}
