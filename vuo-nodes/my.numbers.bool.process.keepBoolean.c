/**
 * @file
 * my.numbers.bool.process.keepBoolean node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_KeepBoolean
 * - Category: Operators/Lib/numbers/bool/process
 * - Source: external/tixl/Operators/Lib/numbers/bool/process/KeepBoolean.cs
 * - Primary output(s): VuoBoolean Result, VuoReal TimeSinceFreeze (ColorForValues #868C8D)
 *
 * bounded freeze adapter. ProofValue is a Vuo-only numeric tap for the Batch 47 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_KeepBoolean",
					 "description" : "TiXL KeepBoolean bounded freeze adapter. Source: external/tixl/Operators/Lib/numbers/bool/process/KeepBoolean.cs. Category: Operators/Lib/numbers/bool/process. Primary output(s): VuoBoolean Result, VuoReal TimeSinceFreeze (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoReal, {"name":"TimeSinceFreeze"}) timeSinceFreeze,
		VuoOutputData(VuoReal, {"name":"ProofValue"}) proofValue
)
{
	VuoReal proof = 0.0;
	(void)renderTick; (void)mode; (void)timeMode; (void)freeze; (void)uniqueForChild;
	*result = freeze ? value : trigger; *timeSinceFreeze = freeze ? fmax(0.0, timeValue) : 0.0; proof = (*result ? 1.0 : 0.0) + *timeSinceFreeze;
	*proofValue = proof;
}
