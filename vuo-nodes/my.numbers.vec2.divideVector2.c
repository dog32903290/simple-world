/**
 * @file
 * my.numbers.vec2.divideVector2 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_DivideVector2
 * - Category: Operators/Lib/numbers/vec2
 * - Source: external/tixl/Operators/Lib/numbers/vec2/DivideVector2.cs
 * - Primary output(s): VuoPoint2d Result (ColorForValues #868C8D)
 *
 * exact safe-divide vector adapter. ProofValue is a Vuo-only numeric tap for the Batch 49 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_DivideVector2",
					 "description" : "TiXL DivideVector2 exact safe-divide vector adapter. Source: external/tixl/Operators/Lib/numbers/vec2/DivideVector2.cs. Category: Operators/Lib/numbers/vec2. Primary output(s): VuoPoint2d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint2d_make(safeDiv(input1.x, input2.x), safeDiv(input1.y, input2.y)); proof = length2(*result);
	*proofValue = proof;
}
