/**
 * @file
 * my.numbers.vec3.addVec3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_AddVec3
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/AddVec3.cs
 * - Primary output(s): VuoPoint3d Input1, VuoPoint3d Result (ColorForValues #868C8D)
 *
...
VuoModuleMetadata({
					 "title" : "my_AddVec3",
					 "description" : "TiXL AddVec3 exact vector adapter. Source: external/tixl/Operators/Lib/numbers/vec3/AddVec3.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Input1, VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoPoint3d, {"name":"Input1"}) input1Out,
...
)
{
	...
	*input1Out = vectorA; *result = add3(vectorA, vectorB); proof = length3(*result);
	*proofValue = proof;
}
