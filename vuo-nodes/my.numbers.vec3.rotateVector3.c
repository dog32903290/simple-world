/**
 * @file
 * my.numbers.vec3.rotateVector3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_RotateVector3
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/RotateVector3.cs
 * - Primary output(s): VuoPoint3d Result (ColorForValues #868C8D)
 *
 * bounded axis rotation adapter. ProofValue is a Vuo-only numeric tap for the Batch 51 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_RotateVector3",
					 "description" : "TiXL RotateVector3 bounded axis rotation adapter. Source: external/tixl/Operators/Lib/numbers/vec3/RotateVector3.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = rotateAroundAxis(vectorA, axisInput, angleInput * scale); proof = length3(*result);
	*proofValue = proof;
}
