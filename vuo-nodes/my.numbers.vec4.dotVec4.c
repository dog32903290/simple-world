/**
 * @file
 * my.numbers.vec4.dotVec4 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_DotVec4
 * - Category: Operators/Lib/numbers/vec4
 * - Source: external/tixl/Operators/Lib/numbers/vec4/DotVec4.cs
 * - Primary output(s): VuoReal Result (ColorForValues #868C8D)
 *
 * exact vector adapter. ProofValue is a Vuo-only numeric tap for the Batch 53 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_DotVec4",
					 "description" : "TiXL DotVec4 exact vector adapter. Source: external/tixl/Operators/Lib/numbers/vec4/DotVec4.cs. Category: Operators/Lib/numbers/vec4. Primary output(s): VuoReal Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = dot4(vector4A, vector4B); proof = *result;
	*proofValue = proof;
}
