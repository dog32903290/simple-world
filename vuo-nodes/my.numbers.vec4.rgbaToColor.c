/**
 * @file
 * my.numbers.vec4.rgbaToColor node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_RgbaToColor
 * - Category: Operators/Lib/numbers/vec4
 * - Source: external/tixl/Operators/Lib/numbers/vec4/RgbaToColor.cs
 * - Primary output(s): VuoPoint4d Result (ColorForValues #868C8D)
 *
...
VuoModuleMetadata({
					 "title" : "my_RgbaToColor",
					 "description" : "TiXL RgbaToColor exact Vector4 adapter. Source: external/tixl/Operators/Lib/numbers/vec4/RgbaToColor.cs. Category: Operators/Lib/numbers/vec4. Primary output(s): VuoPoint4d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoPoint4d, {"name":"Result"}) result,
...
)
{
	...
	*result = VuoPoint4d_make(r, g, b, a); proof = length4(*result);
	*proofValue = proof;
}
