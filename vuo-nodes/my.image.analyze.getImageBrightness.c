/**
 * @file
 * my.image.analyze.getImageBrightness node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_GetImageBrightness
 * - Category: Operators/Lib/image/analyze
 * - Source: external/tixl/Operators/Lib/image/analyze/GetImageBrightness.cs
 * - Default evidence: external/tixl/Operators/Lib/image/analyze/GetImageBrightness.t3
 * - Primary output evidence: BrightnessImage; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `cs-GetImageBrightness.hlsl`.
...
VuoModuleMetadata({
 "title":"my_GetImageBrightness",
 "description":"TiXL GetImageBrightness bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/analyze/GetImageBrightness.cs. Category: Operators/Lib/image/analyze. Primary output evidence: BrightnessImage. Shader/resource evidence: cs-GetImageBrightness.hlsl.",
 "keywords":["tixl","image","Texture2D","GetImageBrightness","cs-GetImageBrightness.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_GetImageBrightness Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
...
(
		...
		VuoInputData(VuoImage, {"name":"Texture2d"}) texture2d,
