/**
 * @file
 * my.numbers.anim.utils.findKeyframes node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_FindKeyframes
 * - Category: Operators/Lib/numbers/anim/utils
 * - Source: external/tixl/Operators/Lib/numbers/anim/utils/FindKeyframes.cs
 * - Primary output(s): VuoInteger KeyframeCount, VuoReal Time, VuoReal Value (ColorForValues #868C8D)
 *
 * side-effect bounded keyframe adapter. ProofValue is a Vuo-only numeric tap for the Batch 46 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_FindKeyframes",
					 "description" : "TiXL FindKeyframes side-effect bounded keyframe adapter. Source: external/tixl/Operators/Lib/numbers/anim/utils/FindKeyframes.cs. Category: Operators/Lib/numbers/anim/utils. Primary output(s): VuoInteger KeyframeCount, VuoReal Time, VuoReal Value (ColorForValues #868C8D). ProofValue is Vuo-only.",
					 "keywords" : [ "tixl", "numbers", "ColorForValues", "#868C8D", "batch46" ],
					 "version" : "1.0.0",
					 "dependencies" : [ ],
				 });
...
(
		...
		VuoOutputData(VuoInteger, {"name":"KeyframeCount"}) keyframeCount,
		VuoOutputData(VuoReal, {"name":"Time"}) timeOut,
		VuoOutputData(VuoReal, {"name":"Value"}) valueOut,
		VuoOutputData(VuoReal, {"name":"ProofValue"}) proofValue
)
{
	VuoReal proof = 0.0;
	(void)renderTick; (void)mode; (void)timeMode; (void)freeze; (void)uniqueForChild;
	*keyframeCount = frameCount < 0 ? 0 : frameCount; *timeOut = timeValue; *valueOut = mixReal(a, b, clampReal(timeValue, 0.0, 1.0)); proof = *valueOut;
	*proofValue = proof;
}
