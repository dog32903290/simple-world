/**
 * @file
 * my.numbers.bool.process.delayTriggerChange node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_DelayTriggerChange
 * - Category: Operators/Lib/numbers/bool/process
 * - Source: external/tixl/Operators/Lib/numbers/bool/process/DelayTriggerChange.cs
 * - Primary output(s): VuoBoolean DelayedTrigger, VuoReal RemainingTime (ColorForValues #868C8D)
 *
 * bounded stateless delay adapter. ProofValue is a Vuo-only numeric tap for the Batch 47 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_DelayTriggerChange",
					 "description" : "TiXL DelayTriggerChange bounded stateless delay adapter. Source: external/tixl/Operators/Lib/numbers/bool/process/DelayTriggerChange.cs. Category: Operators/Lib/numbers/bool/process. Primary output(s): VuoBoolean DelayedTrigger, VuoReal RemainingTime (ColorForValues #868C8D). ProofValue is Vuo-only.",
					 "keywords" : [ "tixl", "numbers", "ColorForValues", "#868C8D", "batch47" ],
					 "version" : "1.0.0",
					 "dependencies" : [ ],
				 });
...
(
		...
		VuoOutputData(VuoBoolean, {"name":"DelayedTrigger"}) delayedTrigger,
		VuoOutputData(VuoReal, {"name":"RemainingTime"}) remainingTime,
		VuoOutputData(VuoReal, {"name":"ProofValue"}) proofValue
)
{
	VuoReal proof = 0.0;
	(void)renderTick; (void)mode; (void)timeMode; (void)freeze; (void)uniqueForChild;
	*delayedTrigger = delayDuration <= 0.0 ? trigger : false; *remainingTime = fmax(0.0, delayDuration); proof = (*delayedTrigger ? 1.0 : 0.0) + *remainingTime;
	*proofValue = proof;
}
