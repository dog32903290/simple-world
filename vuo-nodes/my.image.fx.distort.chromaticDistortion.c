/**
 * @file
 * my.image.fx.distort.chromaticDistortion node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_ChromaticDistortion
 * - Category: Operators/Lib/image/fx/distort
 * - Source: external/tixl/Operators/Lib/image/fx/distort/ChromaticDistortion.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/distort/ChromaticDistortion.t3
 * - Primary output evidence: Output; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `ChromaticDistortion.hlsl`.
...
VuoModuleMetadata({
 "title":"my_ChromaticDistortion",
 "description":"TiXL ChromaticDistortion bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/distort/ChromaticDistortion.cs. Category: Operators/Lib/image/fx/distort. Primary output evidence: Output. Shader/resource evidence: ChromaticDistortion.hlsl.",
 "keywords":["tixl","image","Texture2D","ChromaticDistortion","ChromaticDistortion.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_ChromaticDistortion Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
