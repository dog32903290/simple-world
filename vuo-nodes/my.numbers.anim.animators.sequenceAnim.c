/**
 * @file
 * my.numbers.anim.animators.sequenceAnim node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_SequenceAnim
 * - Category: Operators/Lib/numbers/anim/animators
 * - Source: external/tixl/Operators/Lib/numbers/anim/animators/SequenceAnim.cs
 * - Primary output(s): VuoReal Result, VuoBoolean WasStep (ColorForValues #868C8D)
 *
 * bounded sequence adapter. ProofValue is a Vuo-only numeric tap for the Batch 54 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_SequenceAnim",
					 "description" : "TiXL SequenceAnim bounded sequence adapter. Source: external/tixl/Operators/Lib/numbers/anim/animators/SequenceAnim.cs. Category: Operators/Lib/numbers/anim/animators. Primary output(s): VuoReal Result, VuoBoolean WasStep (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoBoolean, {"name":"WasStep"}) wasStep,
...
)
{
	...
	VuoInteger step = (VuoInteger)floor((timeValue + phase) * fmax(1.0, rate)); *wasStep = step != 0 && step % 2 == 0; *result = mixReal(minValue, maxValue, (VuoReal)(step % 8) / 7.0) + bias; proof = *result + (*wasStep ? 0.25 : 0.0);
	*proofValue = proof;
}
