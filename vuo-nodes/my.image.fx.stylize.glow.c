/**
 * @file
 * my.image.fx.stylize.glow node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Glow
 * - Category: Operators/Lib/image/fx/stylize
 * - Source: external/tixl/Operators/Lib/image/fx/stylize/Glow.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/stylize/Glow.t3
 * - Primary output evidence: ImgOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `Glow.cs`.
...
VuoModuleMetadata({
 "title":"my_Glow",
 "description":"TiXL Glow bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/stylize/Glow.cs. Category: Operators/Lib/image/fx/stylize. Primary output evidence: ImgOutput. Shader/resource evidence: Glow.cs.",
 "keywords":["tixl","image","Texture2D","Glow","Glow.cs","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_Glow Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
...
(
		...
		VuoInputData(VuoImage, {"name":"Texture"}) texture,
