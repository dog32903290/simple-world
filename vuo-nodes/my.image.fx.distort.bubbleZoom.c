/**
 * @file
 * my.image.fx.distort.bubbleZoom node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_BubbleZoom
 * - Category: Operators/Lib/image/fx/distort
 * - Source: external/tixl/Operators/Lib/image/fx/distort/BubbleZoom.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/distort/BubbleZoom.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `BubbleZoom.hlsl`.
...
VuoModuleMetadata({
 "title":"my_BubbleZoom",
 "description":"TiXL BubbleZoom bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/distort/BubbleZoom.cs. Category: Operators/Lib/image/fx/distort. Primary output evidence: TextureOutput. Shader/resource evidence: BubbleZoom.hlsl.",
 "keywords":["tixl","image","Texture2D","BubbleZoom","BubbleZoom.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_BubbleZoom Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
