/**
 * @file
 * my.numbers.anim.utils.setKeyframes node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_SetKeyframes
 * - Category: Operators/Lib/numbers/anim/utils
 * - Source: external/tixl/Operators/Lib/numbers/anim/utils/SetKeyframes.cs
 * - Primary output(s): VuoReal CurrentValue (ColorForValues #868C8D)
 *
...
VuoModuleMetadata({
					 "title" : "my_SetKeyframes",
					 "description" : "TiXL SetKeyframes side-effect bounded keyframe adapter. Source: external/tixl/Operators/Lib/numbers/anim/utils/SetKeyframes.cs. Category: Operators/Lib/numbers/anim/utils. Primary output(s): VuoReal CurrentValue (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoReal, {"name":"CurrentValue"}) currentValue,
		VuoOutputData(VuoReal, {"name":"ProofValue"}) proofValue
)
{
	VuoReal proof = 0.0;
	(void)renderTick; (void)mode; (void)timeMode; (void)freeze; (void)uniqueForChild;
	*currentValue = mixReal(a, b, clampReal(timeValue, 0.0, 1.0)); proof = *currentValue;
	*proofValue = proof;
}
