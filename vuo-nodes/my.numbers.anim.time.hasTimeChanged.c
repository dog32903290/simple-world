/**
 * @file
 * my.numbers.anim.time.hasTimeChanged node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_HasTimeChanged
 * - Category: Operators/Lib/numbers/anim/time
 * - Source: external/tixl/Operators/Lib/numbers/anim/time/HasTimeChanged.cs
 * - Primary output(s): VuoReal DeltaTime, VuoBoolean HasChanged (ColorForValues #868C8D)
 *
 * bounded stateless change adapter. ProofValue is a Vuo-only numeric tap for the Batch 45 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_HasTimeChanged",
					 "description" : "TiXL HasTimeChanged bounded stateless change adapter. Source: external/tixl/Operators/Lib/numbers/anim/time/HasTimeChanged.cs. Category: Operators/Lib/numbers/anim/time. Primary output(s): VuoReal DeltaTime, VuoBoolean HasChanged (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*deltaTime = timeValue - a; *hasChanged = fabs(*deltaTime) > 0.00001; proof = *hasChanged ? clampReal(fabs(*deltaTime), 0.0, 10.0) : 0.0;
	*proofValue = proof;
}
