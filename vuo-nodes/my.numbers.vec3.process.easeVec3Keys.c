/**
 * @file
 * my.numbers.vec3.process.easeVec3Keys node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_EaseVec3Keys
 * - Category: Operators/Lib/numbers/vec3/process
 * - Source: external/tixl/Operators/Lib/numbers/vec3/process/EaseVec3Keys.cs
 * - Primary output(s): VuoPoint3d Result (ColorForValues #868C8D)
 *
 * bounded vector key easing adapter. ProofValue is a Vuo-only numeric tap for the Batch 52 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_EaseVec3Keys",
					 "description" : "TiXL EaseVec3Keys bounded vector key easing adapter. Source: external/tixl/Operators/Lib/numbers/vec3/process/EaseVec3Keys.cs. Category: Operators/Lib/numbers/vec3/process. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
					 "keywords" : [ "tixl", "numbers", "ColorForValues", "#868C8D", "batch52" ],
					 "version" : "1.0.0",
					 "dependencies" : [ ],
				 });
...
)
{
	...
	*result = mix3(vectorB, vectorA, ease01(clampReal(f, 0.0, 1.0), interpolation)); proof = length3(*result);
	*proofValue = proof;
}
