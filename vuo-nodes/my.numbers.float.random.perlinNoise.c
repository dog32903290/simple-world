/**
 * @file
 * my.numbers.float.random.perlinNoise node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_PerlinNoise
 * - Category: Operators/Lib/numbers/float/random
 * - Source: external/tixl/Operators/Lib/numbers/float/random/PerlinNoise.cs
 * - Primary output(s): VuoReal Result (ColorForValues #868C8D)
 *
 * bounded value-noise adapter. ProofValue is a Vuo-only numeric tap for the Batch 48 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_PerlinNoise",
					 "description" : "TiXL PerlinNoise bounded value-noise adapter. Source: external/tixl/Operators/Lib/numbers/float/random/PerlinNoise.cs. Category: Operators/Lib/numbers/float/random. Primary output(s): VuoReal Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = noise2(input1.x * scale + a, input1.y * scale + b, seed); proof = *result;
	*proofValue = proof;
}
