/**
 * @file
 * my.numbers.anim.animators.animInt node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_AnimInt
 * - Category: Operators/Lib/numbers/anim/animators
 * - Source: external/tixl/Operators/Lib/numbers/anim/animators/AnimInt.cs
 * - Primary output(s): VuoInteger Result, VuoBoolean WasHit (ColorForValues #868C8D)
 *
 * bounded integer LFO adapter. ProofValue is a Vuo-only numeric tap for the Batch 54 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_AnimInt",
					 "description" : "TiXL AnimInt bounded integer LFO adapter. Source: external/tixl/Operators/Lib/numbers/anim/animators/AnimInt.cs. Category: Operators/Lib/numbers/anim/animators. Primary output(s): VuoInteger Result, VuoBoolean WasHit (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoBoolean, {"name":"WasHit"}) wasHit,
...
)
{
	...
	VuoReal wave = 0.5 + 0.5 * sin((timeValue + phase) * rate * 6.2831853); VuoInteger mod = modulo <= 0 ? 1 : modulo; *result = (VuoInteger)floor(wave * mod); *wasHit = *result == 0; proof = (VuoReal)*result + (*wasHit ? 0.5 : 0.0);
	*proofValue = proof;
}
