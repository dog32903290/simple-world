/**
 * @file
 * my.image.use.combineMaterialChannels2 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_CombineMaterialChannels2
 * - Category: Operators/Lib/image/use
 * - Source: external/tixl/Operators/Lib/image/use/CombineMaterialChannels2.cs
 * - Default: SelectChannel_R=0, SelectChannel_G=6, SelectChannel_B=12, SelectAlphaChannel=4 from CombineMaterialChannels2.t3
...
VuoModuleMetadata({
					 "title" : "my_CombineMaterialChannels2",
					 "description" : "TiXL CombineMaterialChannels2 Vuo shader adapter. Source: external/tixl/Operators/Lib/image/use/CombineMaterialChannels2.cs. Category: Operators/Lib/image/use. Primary output: Texture2D Output (ColorForTextures #9F008A). Default: SelectChannel_R=0, SelectChannel_G=6, SelectChannel_B=12, SelectAlphaChannel=4.",
					 "keywords" : [ "tixl", "texture2d", "image", "material", "channel", "img-combine-3.hlsl", "ColorForTextures", "#9F008A" ],
					 "version" : "1.0.0",
...
static const char *fragmentShaderSource = VUOSHADER_GLSL_SOURCE(120,
	...
	float selectedChannel(vec4 a, vec4 b, vec4 c, int select)
	{
		...
		if (mode == 0) return source.r;
		if (mode == 1) return source.g;
		if (mode == 2) return source.b;
		if (mode == 3) return (source.r + source.g + source.b) / 3.0;
	...
	void main()
	{
		...
		gl_FragColor = vec4(selectedChannel(a, b, c, selectChannelR), selectedChannel(a, b, c, selectChannelG), selectedChannel(a, b, c, selectChannelB), alpha);
	}
);
...
struct nodeInstanceData * nodeInstanceInit(void)
{
	...
	instance->shader = VuoShader_make("my_CombineMaterialChannels2 Shader");
