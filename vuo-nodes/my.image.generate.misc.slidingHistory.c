/**
 * @file
 * my.image.generate.misc.slidingHistory node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_SlidingHistory
 * - Category: Operators/Lib/image/generate/misc
 * - Source: external/tixl/Operators/Lib/image/generate/misc/SlidingHistory.cs
 * - Default evidence: external/tixl/Operators/Lib/image/generate/misc/SlidingHistory.t3
 * - Primary output evidence: Output; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `SlidingHistory.hlsl`.
...
VuoModuleMetadata({
 "title":"my_SlidingHistory",
 "description":"TiXL SlidingHistory bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/generate/misc/SlidingHistory.cs. Category: Operators/Lib/image/generate/misc. Primary output evidence: Output. Shader/resource evidence: SlidingHistory.hlsl.",
 "keywords":["tixl","image","Texture2D","SlidingHistory","SlidingHistory.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_SlidingHistory Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
