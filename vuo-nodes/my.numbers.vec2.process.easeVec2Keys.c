/**
 * @file
 * my.numbers.vec2.process.easeVec2Keys node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_EaseVec2Keys
 * - Category: Operators/Lib/numbers/vec2/process
 * - Source: external/tixl/Operators/Lib/numbers/vec2/process/EaseVec2Keys.cs
 * - Primary output(s): VuoPoint2d Result (ColorForValues #868C8D)
 *
 * bounded vector key easing adapter. ProofValue is a Vuo-only numeric tap for the Batch 50 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_EaseVec2Keys",
					 "description" : "TiXL EaseVec2Keys bounded vector key easing adapter. Source: external/tixl/Operators/Lib/numbers/vec2/process/EaseVec2Keys.cs. Category: Operators/Lib/numbers/vec2/process. Primary output(s): VuoPoint2d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	VuoPoint2d target = VuoPoint2d_make(input2d.y, input2d.x); *result = mix2(input2d, target, ease01(clampReal(f, 0.0, 1.0), interpolation)); proof = length2(*result);
	*proofValue = proof;
}
