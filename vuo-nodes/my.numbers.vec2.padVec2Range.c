/**
 * @file
 * my.numbers.vec2.padVec2Range node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_PadVec2Range
 * - Category: Operators/Lib/numbers/vec2
 * - Source: external/tixl/Operators/Lib/numbers/vec2/PadVec2Range.cs
 * - Primary output(s): VuoPoint2d Result (ColorForValues #868C8D)
 *
 * bounded range adapter. ProofValue is a Vuo-only numeric tap for the Batch 49 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_PadVec2Range",
					 "description" : "TiXL PadVec2Range bounded range adapter. Source: external/tixl/Operators/Lib/numbers/vec2/PadVec2Range.cs. Category: Operators/Lib/numbers/vec2. Primary output(s): VuoPoint2d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint2d_make(input1.x + a, input1.y + b); proof = length2(*result);
	*proofValue = proof;
}
