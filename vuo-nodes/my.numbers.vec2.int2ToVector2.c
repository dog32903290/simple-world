/**
 * @file
 * my.numbers.vec2.int2ToVector2 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Int2ToVector2
 * - Category: Operators/Lib/numbers/vec2
 * - Source: external/tixl/Operators/Lib/numbers/vec2/Int2ToVector2.cs
...
VuoModuleMetadata({
					 "title" : "my_Int2ToVector2",
					 "description" : "TiXL Int2ToVector2 exact vector adapter. Source: external/tixl/Operators/Lib/numbers/vec2/Int2ToVector2.cs. Category: Operators/Lib/numbers/vec2. Primary output(s): VuoPoint2d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint2d_make((VuoReal)frameCount, (VuoReal)index); proof = length2(*result);
	*proofValue = proof;
}
