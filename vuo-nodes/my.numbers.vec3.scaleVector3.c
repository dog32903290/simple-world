/**
 * @file
 * my.numbers.vec3.scaleVector3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_ScaleVector3
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/ScaleVector3.cs
...
VuoModuleMetadata({
					 "title" : "my_ScaleVector3",
					 "description" : "TiXL ScaleVector3 exact vector adapter. Source: external/tixl/Operators/Lib/numbers/vec3/ScaleVector3.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint3d_make(vectorA.x * vectorB.x * scale, vectorA.y * vectorB.y * scale, vectorA.z * vectorB.z * scale); proof = length3(*result);
	*proofValue = proof;
}
