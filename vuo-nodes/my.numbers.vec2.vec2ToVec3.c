/**
 * @file
 * my.numbers.vec2.vec2ToVec3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Vec2ToVec3
 * - Category: Operators/Lib/numbers/vec2
 * - Source: external/tixl/Operators/Lib/numbers/vec2/Vec2ToVec3.cs
...
VuoModuleMetadata({
					 "title" : "my_Vec2ToVec3",
					 "description" : "TiXL Vec2ToVec3 exact vector adapter. Source: external/tixl/Operators/Lib/numbers/vec2/Vec2ToVec3.cs. Category: Operators/Lib/numbers/vec2. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint3d_make(input1.x, input1.y, a); proof = sqrt(result->x * result->x + result->y * result->y + result->z * result->z);
	*proofValue = proof;
}
