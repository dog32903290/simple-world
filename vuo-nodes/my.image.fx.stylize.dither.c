/**
 * @file
 * my.image.fx.stylize.dither node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Dither
 * - Category: Operators/Lib/image/fx/stylize
 * - Source: external/tixl/Operators/Lib/image/fx/stylize/Dither.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/stylize/Dither.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `Dither.hlsl`.
...
VuoModuleMetadata({
 "title":"my_Dither",
 "description":"TiXL Dither bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/stylize/Dither.cs. Category: Operators/Lib/image/fx/stylize. Primary output evidence: TextureOutput. Shader/resource evidence: Dither.hlsl.",
 "keywords":["tixl","image","Texture2D","Dither","Dither.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_Dither Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
