/**
 * @file
 * my.numbers.anim.time.runTime node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_RunTime
 * - Category: Operators/Lib/numbers/anim/time
 * - Source: external/tixl/Operators/Lib/numbers/anim/time/RunTime.cs
 * - Primary output(s): VuoReal TimeInSeconds (ColorForValues #868C8D)
 *
 * bounded runtime adapter. ProofValue is a Vuo-only numeric tap for the Batch 45 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_RunTime",
					 "description" : "TiXL RunTime bounded runtime adapter. Source: external/tixl/Operators/Lib/numbers/anim/time/RunTime.cs. Category: Operators/Lib/numbers/anim/time. Primary output(s): VuoReal TimeInSeconds (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoReal, {"name":"TimeInSeconds"}) timeInSeconds,
		VuoOutputData(VuoReal, {"name":"ProofValue"}) proofValue
)
{
	VuoReal proof = 0.0;
	(void)renderTick; (void)mode; (void)timeMode; (void)freeze; (void)uniqueForChild;
	*timeInSeconds = fmax(0.0, timeValue * fmax(0.0, speedFactor)); proof = *timeInSeconds;
	*proofValue = proof;
}
