/**
 * @file
 * my.numbers.anim.time.abletonLinkSync node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_AbletonLinkSync
 * - Category: Operators/Lib/numbers/anim/time
 * - Source: external/tixl/Operators/Lib/numbers/anim/time/AbletonLinkSync.cs
 * - Primary output(s): VuoBoolean IsConnected, VuoReal Result, VuoReal Tempo (ColorForValues #868C8D)
 *
 * side-effect bounded adapter. ProofValue is a Vuo-only numeric tap for the Batch 45 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_AbletonLinkSync",
					 "description" : "TiXL AbletonLinkSync side-effect bounded adapter. Source: external/tixl/Operators/Lib/numbers/anim/time/AbletonLinkSync.cs. Category: Operators/Lib/numbers/anim/time. Primary output(s): VuoBoolean IsConnected, VuoReal Result, VuoReal Tempo (ColorForValues #868C8D). ProofValue is Vuo-only.",
					 "keywords" : [ "tixl", "numbers", "ColorForValues", "#868C8D", "batch45" ],
					 "version" : "1.0.0",
					 "dependencies" : [ ],
				 });
...
(
		...
		VuoOutputData(VuoBoolean, {"name":"IsConnected"}) isConnected,
		VuoOutputData(VuoReal, {"name":"Result"}) result,
		VuoOutputData(VuoReal, {"name":"Tempo"}) tempo,
		VuoOutputData(VuoReal, {"name":"ProofValue"}) proofValue
)
{
	VuoReal proof = 0.0;
	(void)renderTick; (void)mode; (void)timeMode; (void)freeze; (void)uniqueForChild;
	*isConnected = false; *result = timeValue; *tempo = 120.0 + speedFactor * 10.0; proof = *result;
	*proofValue = proof;
}
