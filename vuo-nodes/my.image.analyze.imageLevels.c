/**
 * @file
 * my.image.analyze.imageLevels node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_ImageLevels
 * - Category: Operators/Lib/image/analyze
 * - Source: external/tixl/Operators/Lib/image/analyze/ImageLevels.cs
 * - Default evidence: external/tixl/Operators/Lib/image/analyze/ImageLevels.t3
 * - Primary output evidence: Output; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `ImageLevels.hlsl`.
...
VuoModuleMetadata({
 "title":"my_ImageLevels",
 "description":"TiXL ImageLevels bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/analyze/ImageLevels.cs. Category: Operators/Lib/image/analyze. Primary output evidence: Output. Shader/resource evidence: ImageLevels.hlsl.",
 "keywords":["tixl","image","Texture2D","ImageLevels","ImageLevels.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_ImageLevels Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
