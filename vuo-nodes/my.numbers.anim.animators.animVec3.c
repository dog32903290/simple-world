/**
 * @file
 * my.numbers.anim.animators.animVec3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_AnimVec3
 * - Category: Operators/Lib/numbers/anim/animators
 * - Source: external/tixl/Operators/Lib/numbers/anim/animators/AnimVec3.cs
 * - Primary output(s): VuoPoint3d Result (ColorForValues #868C8D)
 *
 * bounded vector LFO adapter. ProofValue is a Vuo-only numeric tap for the Batch 54 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_AnimVec3",
					 "description" : "TiXL AnimVec3 bounded vector LFO adapter. Source: external/tixl/Operators/Lib/numbers/anim/animators/AnimVec3.cs. Category: Operators/Lib/numbers/anim/animators. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint3d_make(vectorA.x + amplitude * sin((timeValue + phase) * rate * 6.2831853), vectorA.y + amplitude * cos((timeValue + phase) * rate * 6.2831853), vectorA.z + amplitude * sin((timeValue + phase + 0.25) * rate * 6.2831853)); proof = length3(*result);
	*proofValue = proof;
}
