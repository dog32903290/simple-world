/**
 * @file
 * my.numbers.vec3.dampVec3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_DampVec3
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/DampVec3.cs
 * - Primary output(s): VuoPoint3d Result (ColorForValues #868C8D)
 *
 * bounded vector damping adapter. ProofValue is a Vuo-only numeric tap for the Batch 51 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_DampVec3",
					 "description" : "TiXL DampVec3 bounded vector damping adapter. Source: external/tixl/Operators/Lib/numbers/vec3/DampVec3.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = mix3(vectorA, vectorB, clampReal(damping, 0.0, 1.0)); proof = length3(*result);
	*proofValue = proof;
}
