/**
 * @file
 * my.image.fx.stylize.chromaticAbberation node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_ChromaticAbberation
 * - Category: Operators/Lib/image/fx/stylize
 * - Source: external/tixl/Operators/Lib/image/fx/stylize/ChromaticAbberation.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/stylize/ChromaticAbberation.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `ChromaticAbberation.hlsl`.
...
VuoModuleMetadata({
 "title":"my_ChromaticAbberation",
 "description":"TiXL ChromaticAbberation bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/stylize/ChromaticAbberation.cs. Category: Operators/Lib/image/fx/stylize. Primary output evidence: TextureOutput. Shader/resource evidence: ChromaticAbberation.hlsl.",
 "keywords":["tixl","image","Texture2D","ChromaticAbberation","ChromaticAbberation.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_ChromaticAbberation Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
