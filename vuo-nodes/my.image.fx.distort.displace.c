/**
 * @file
 * my.image.fx.distort.displace node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Displace
 * - Category: Operators/Lib/image/fx/distort
 * - Source: external/tixl/Operators/Lib/image/fx/distort/Displace.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/distort/Displace.t3
 * - Primary output evidence: Output; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `Displace.hlsl`.
...
VuoModuleMetadata({
 "title":"my_Displace",
 "description":"TiXL Displace bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/distort/Displace.cs. Category: Operators/Lib/image/fx/distort. Primary output evidence: Output. Shader/resource evidence: Displace.hlsl.",
 "keywords":["tixl","image","Texture2D","Displace","Displace.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_Displace Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
