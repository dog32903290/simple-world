/**
 * @file
 * my.image.analyze.compareImages node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_CompareImages
 * - Category: Operators/Lib/image/analyze
 * - Source: external/tixl/Operators/Lib/image/analyze/CompareImages.cs
 * - Default evidence: external/tixl/Operators/Lib/image/analyze/CompareImages.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `CompareImages.cs`.
...
VuoModuleMetadata({
 "title":"my_CompareImages",
 "description":"TiXL CompareImages bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/analyze/CompareImages.cs. Category: Operators/Lib/image/analyze. Primary output evidence: TextureOutput. Shader/resource evidence: CompareImages.cs.",
 "keywords":["tixl","image","Texture2D","CompareImages","CompareImages.cs","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_CompareImages Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
