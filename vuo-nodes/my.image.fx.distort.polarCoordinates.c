/**
 * @file
 * my.image.fx.distort.polarCoordinates node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_PolarCoordinates
 * - Category: Operators/Lib/image/fx/distort
 * - Source: external/tixl/Operators/Lib/image/fx/distort/PolarCoordinates.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/distort/PolarCoordinates.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `PolarCoordinates.hlsl`.
...
VuoModuleMetadata({
 "title":"my_PolarCoordinates",
 "description":"TiXL PolarCoordinates bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/distort/PolarCoordinates.cs. Category: Operators/Lib/image/fx/distort. Primary output evidence: TextureOutput. Shader/resource evidence: PolarCoordinates.hlsl.",
 "keywords":["tixl","image","Texture2D","PolarCoordinates","PolarCoordinates.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_PolarCoordinates Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
