/**
 * @file
 * my.numbers.vec2.hasVec2Changed node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_HasVec2Changed
 * - Category: Operators/Lib/numbers/vec2
 * - Source: external/tixl/Operators/Lib/numbers/vec2/HasVec2Changed.cs
 * - Primary output(s): VuoPoint2d Delta, VuoBoolean HasChanged (ColorForValues #868C8D)
 *
 * bounded stateless change adapter. ProofValue is a Vuo-only numeric tap for the Batch 49 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_HasVec2Changed",
					 "description" : "TiXL HasVec2Changed bounded stateless change adapter. Source: external/tixl/Operators/Lib/numbers/vec2/HasVec2Changed.cs. Category: Operators/Lib/numbers/vec2. Primary output(s): VuoPoint2d Delta, VuoBoolean HasChanged (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoPoint2d, {"name":"Delta"}) delta,
...
)
{
	...
	*delta = VuoPoint2d_make(input1.x - input2.x, input1.y - input2.y); *hasChanged = length2(*delta) > 0.00001; proof = *hasChanged ? length2(*delta) : 0.0;
	*proofValue = proof;
}
