/**
 * @file
 * my.numbers.anim.animators.animBoolean node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_AnimBoolean
 * - Category: Operators/Lib/numbers/anim/animators
 * - Source: external/tixl/Operators/Lib/numbers/anim/animators/AnimBoolean.cs
 * - Primary output(s): VuoBoolean TriggerOutput (ColorForValues #868C8D)
 *
 * bounded LFO bool adapter. ProofValue is a Vuo-only numeric tap for the Batch 54 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_AnimBoolean",
					 "description" : "TiXL AnimBoolean bounded LFO bool adapter. Source: external/tixl/Operators/Lib/numbers/anim/animators/AnimBoolean.cs. Category: Operators/Lib/numbers/anim/animators. Primary output(s): VuoBoolean TriggerOutput (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoBoolean, {"name":"TriggerOutput"}) triggerOutput,
...
)
{
	...
	*triggerOutput = sin((timeValue + phase) * rate * 6.2831853) > 0.0; proof = *triggerOutput ? 1.0 : 0.0;
	*proofValue = proof;
}
