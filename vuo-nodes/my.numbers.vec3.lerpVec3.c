/**
 * @file
 * my.numbers.vec3.lerpVec3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_LerpVec3
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/LerpVec3.cs
...
VuoModuleMetadata({
					 "title" : "my_LerpVec3",
					 "description" : "TiXL LerpVec3 exact vector adapter. Source: external/tixl/Operators/Lib/numbers/vec3/LerpVec3.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = mix3(vectorA, vectorB, clampInput ? clampReal(f, 0.0, 1.0) : f); proof = length3(*result);
	*proofValue = proof;
}
