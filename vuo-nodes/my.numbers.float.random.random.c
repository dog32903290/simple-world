/**
 * @file
 * my.numbers.float.random.random node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Random
 * - Category: Operators/Lib/numbers/float/random
 * - Source: external/tixl/Operators/Lib/numbers/float/random/Random.cs
 * - Primary output(s): VuoReal Result (ColorForValues #868C8D)
 *
 * deterministic random adapter. ProofValue is a Vuo-only numeric tap for the Batch 48 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_Random",
					 "description" : "TiXL Random deterministic random adapter. Source: external/tixl/Operators/Lib/numbers/float/random/Random.cs. Category: Operators/Lib/numbers/float/random. Primary output(s): VuoReal Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = mixReal(minValue, maxValue, hashUnit(seed, uniqueForChild ? 17u : 0u)); proof = *result;
	*proofValue = proof;
}
