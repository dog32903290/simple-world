/**
 * @file
 * my.numbers.anim.animators.oscillateVec2 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_OscillateVec2
 * - Category: Operators/Lib/numbers/anim/animators
 * - Source: external/tixl/Operators/Lib/numbers/anim/animators/OscillateVec2.cs
...
VuoModuleMetadata({
					 "title" : "my_OscillateVec2",
					 "description" : "TiXL OscillateVec2 exact-ish sine vector adapter. Source: external/tixl/Operators/Lib/numbers/anim/animators/OscillateVec2.cs. Category: Operators/Lib/numbers/anim/animators. Primary output(s): VuoPoint2d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint2d_make(input2d.x + amplitude * sin((timeValue + phase) * speedFactor * 6.2831853), input2d.y + amplitude * cos((timeValue + phase) * speedFactor * 6.2831853)); proof = length2(*result);
	*proofValue = proof;
}
