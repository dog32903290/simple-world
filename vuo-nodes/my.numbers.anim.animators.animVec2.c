/**
 * @file
 * my.numbers.anim.animators.animVec2 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_AnimVec2
 * - Category: Operators/Lib/numbers/anim/animators
 * - Source: external/tixl/Operators/Lib/numbers/anim/animators/AnimVec2.cs
...
VuoModuleMetadata({
					 "title" : "my_AnimVec2",
					 "description" : "TiXL AnimVec2 bounded vector LFO adapter. Source: external/tixl/Operators/Lib/numbers/anim/animators/AnimVec2.cs. Category: Operators/Lib/numbers/anim/animators. Primary output(s): VuoPoint2d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint2d_make(input2d.x + amplitude * sin((timeValue + phase) * rate * 6.2831853), input2d.y + amplitude * cos((timeValue + phase) * rate * 6.2831853)); proof = length2(*result);
	*proofValue = proof;
}
