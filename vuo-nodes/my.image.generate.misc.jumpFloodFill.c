/**
 * @file
 * my.image.generate.misc.jumpFloodFill node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_JumpFloodFill
 * - Category: Operators/Lib/image/generate/misc
 * - Source: external/tixl/Operators/Lib/image/generate/misc/JumpFloodFill.cs
 * - Default evidence: external/tixl/Operators/Lib/image/generate/misc/JumpFloodFill.t3
 * - Primary output evidence: ImageOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `img-generate-JumpFloodFill.hlsl`.
...
VuoModuleMetadata({
 "title":"my_JumpFloodFill",
 "description":"TiXL JumpFloodFill bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/generate/misc/JumpFloodFill.cs. Category: Operators/Lib/image/generate/misc. Primary output evidence: ImageOutput. Shader/resource evidence: img-generate-JumpFloodFill.hlsl.",
 "keywords":["tixl","image","Texture2D","JumpFloodFill","img-generate-JumpFloodFill.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_JumpFloodFill Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
...
(
		...
		VuoInputData(VuoImage, {"name":"Image"}) image,
