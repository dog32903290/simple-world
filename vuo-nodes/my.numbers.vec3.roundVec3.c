/**
 * @file
 * my.numbers.vec3.roundVec3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_RoundVec3
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/RoundVec3.cs
 * - Primary output(s): VuoPoint3d Result (ColorForValues #868C8D)
 *
 * bounded vector rounding adapter. ProofValue is a Vuo-only numeric tap for the Batch 51 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_RoundVec3",
					 "description" : "TiXL RoundVec3 bounded vector rounding adapter. Source: external/tixl/Operators/Lib/numbers/vec3/RoundVec3.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint3d_make(roundTo(vectorA.x, precision.x), roundTo(vectorA.y, precision.y), roundTo(vectorA.z, precision.z)); proof = length3(*result);
	*proofValue = proof;
}
