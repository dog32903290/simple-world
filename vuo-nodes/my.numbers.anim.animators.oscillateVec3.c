/**
 * @file
 * my.numbers.anim.animators.oscillateVec3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_OscillateVec3
 * - Category: Operators/Lib/numbers/anim/animators
 * - Source: external/tixl/Operators/Lib/numbers/anim/animators/OscillateVec3.cs
 * - Primary output(s): VuoPoint3d Result (ColorForValues #868C8D)
 *
 * exact-ish sine vector adapter. ProofValue is a Vuo-only numeric tap for the Batch 54 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_OscillateVec3",
					 "description" : "TiXL OscillateVec3 exact-ish sine vector adapter. Source: external/tixl/Operators/Lib/numbers/anim/animators/OscillateVec3.cs. Category: Operators/Lib/numbers/anim/animators. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint3d_make(vectorA.x + amplitude * sin((timeValue + phase) * speedFactor * 6.2831853), vectorA.y + amplitude * cos((timeValue + phase) * speedFactor * 6.2831853), vectorA.z + amplitude * sin((timeValue + phase + 0.5) * speedFactor * 6.2831853)); proof = length3(*result);
	*proofValue = proof;
}
