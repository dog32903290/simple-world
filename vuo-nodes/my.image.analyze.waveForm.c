/**
 * @file
 * my.image.analyze.waveForm node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_WaveForm
 * - Category: Operators/Lib/image/analyze
 * - Source: external/tixl/Operators/Lib/image/analyze/WaveForm.cs
 * - Default evidence: external/tixl/Operators/Lib/image/analyze/WaveForm.t3
 * - Primary output evidence: ImgOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `waveform-cs.hlsl`.
...
VuoModuleMetadata({
 "title":"my_WaveForm",
 "description":"TiXL WaveForm bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/analyze/WaveForm.cs. Category: Operators/Lib/image/analyze. Primary output evidence: ImgOutput. Shader/resource evidence: waveform-cs.hlsl.",
 "keywords":["tixl","image","Texture2D","WaveForm","waveform-cs.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_WaveForm Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
