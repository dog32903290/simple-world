/**
 * @file
 * my.numbers.vec3.pickVector3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_PickVector3
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/PickVector3.cs
 * - Primary output(s): VuoPoint3d Selected (ColorForValues #868C8D)
 *
 * bounded selector adapter. ProofValue is a Vuo-only numeric tap for the Batch 51 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_PickVector3",
					 "description" : "TiXL PickVector3 bounded selector adapter. Source: external/tixl/Operators/Lib/numbers/vec3/PickVector3.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Selected (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoPoint3d, {"name":"Selected"}) selected,
...
)
{
	...
	*selected = index % 2 == 0 ? vectorA : vectorB; proof = length3(*selected);
	*proofValue = proof;
}
