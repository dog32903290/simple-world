/**
 * @file
 * my.numbers.vec2.remapVec2 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_RemapVec2
 * - Category: Operators/Lib/numbers/vec2
 * - Source: external/tixl/Operators/Lib/numbers/vec2/RemapVec2.cs
 * - Primary output(s): VuoPoint2d Result (ColorForValues #868C8D)
 *
 * exact vector remap adapter. ProofValue is a Vuo-only numeric tap for the Batch 49 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_RemapVec2",
					 "description" : "TiXL RemapVec2 exact vector remap adapter. Source: external/tixl/Operators/Lib/numbers/vec2/RemapVec2.cs. Category: Operators/Lib/numbers/vec2. Primary output(s): VuoPoint2d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint2d_make(mixReal(minValue, maxValue, clampReal(input1.x, 0.0, 1.0)), mixReal(minValue, maxValue, clampReal(input1.y, 0.0, 1.0))); proof = length2(*result);
	*proofValue = proof;
}
