/**
 * @file
 * my.numbers.anim.time.setTime node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_SetTime
 * - Category: Operators/Lib/numbers/anim/time
 * - Source: external/tixl/Operators/Lib/numbers/anim/time/SetTime.cs
...
VuoModuleMetadata({
					 "title" : "my_SetTime",
					 "description" : "TiXL SetTime side-effect bounded adapter. Source: external/tixl/Operators/Lib/numbers/anim/time/SetTime.cs. Category: Operators/Lib/numbers/anim/time. Primary output(s): VuoText CommandTrace (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*commandTrace = VuoText_make("SetTime body-layer command trace"); proof = timeValue;
	*proofValue = proof;
}
