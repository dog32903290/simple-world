/**
 * @file
 * my.numbers.bool.process.cacheBoolean node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_CacheBoolean
 * - Category: Operators/Lib/numbers/bool/process
 * - Source: external/tixl/Operators/Lib/numbers/bool/process/CacheBoolean.cs
 * - Primary output(s): VuoBoolean Result (ColorForValues #868C8D)
 *
 * bounded value adapter. ProofValue is a Vuo-only numeric tap for the Batch 47 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_CacheBoolean",
					 "description" : "TiXL CacheBoolean bounded value adapter. Source: external/tixl/Operators/Lib/numbers/bool/process/CacheBoolean.cs. Category: Operators/Lib/numbers/bool/process. Primary output(s): VuoBoolean Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = value; proof = *result ? 1.0 : 0.0;
	*proofValue = proof;
}
