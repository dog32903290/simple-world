/**
 * @file
 * my.numbers.vec4.pickColor node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_PickColor
 * - Category: Operators/Lib/numbers/vec4
 * - Source: external/tixl/Operators/Lib/numbers/vec4/PickColor.cs
 * - Primary output(s): VuoPoint4d Selected (ColorForValues #868C8D)
 *
 * bounded Vector4 selector adapter. ProofValue is a Vuo-only numeric tap for the Batch 53 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_PickColor",
					 "description" : "TiXL PickColor bounded Vector4 selector adapter. Source: external/tixl/Operators/Lib/numbers/vec4/PickColor.cs. Category: Operators/Lib/numbers/vec4. Primary output(s): VuoPoint4d Selected (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoPoint4d, {"name":"Selected"}) selected,
...
)
{
	...
	*selected = index % 2 == 0 ? vector4A : vector4B; proof = length4(*selected);
	*proofValue = proof;
}
