/**
 * @file
 * my.image.fx.stylize.detectEdges node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_DetectEdges
 * - Category: Operators/Lib/image/fx/stylize
 * - Source: external/tixl/Operators/Lib/image/fx/stylize/DetectEdges.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/stylize/DetectEdges.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `DetectEdges.hlsl`.
...
VuoModuleMetadata({
 "title":"my_DetectEdges",
 "description":"TiXL DetectEdges bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/stylize/DetectEdges.cs. Category: Operators/Lib/image/fx/stylize. Primary output evidence: TextureOutput. Shader/resource evidence: DetectEdges.hlsl.",
 "keywords":["tixl","image","Texture2D","DetectEdges","DetectEdges.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_DetectEdges Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
