/**
 * @file
 * my.image.fx.stylize.mosiacTiling node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_MosiacTiling
 * - Category: Operators/Lib/image/fx/stylize
 * - Source: external/tixl/Operators/Lib/image/fx/stylize/MosiacTiling.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/stylize/MosiacTiling.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `MosiacTiling.hlsl`.
...
VuoModuleMetadata({
 "title":"my_MosiacTiling",
 "description":"TiXL MosiacTiling bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/stylize/MosiacTiling.cs. Category: Operators/Lib/image/fx/stylize. Primary output evidence: TextureOutput. Shader/resource evidence: MosiacTiling.hlsl.",
 "keywords":["tixl","image","Texture2D","MosiacTiling","MosiacTiling.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_MosiacTiling Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
