/**
 * @file
 * my.numbers.vec3.eulerToAxisAngle node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_EulerToAxisAngle
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/EulerToAxisAngle.cs
 * - Primary output(s): VuoReal Angle, VuoPoint3d Axis (ColorForValues #868C8D)
 *
 * bounded euler adapter. ProofValue is a Vuo-only numeric tap for the Batch 51 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_EulerToAxisAngle",
					 "description" : "TiXL EulerToAxisAngle bounded euler adapter. Source: external/tixl/Operators/Lib/numbers/vec3/EulerToAxisAngle.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoReal Angle, VuoPoint3d Axis (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoReal, {"name":"Angle"}) angleOut,
		VuoOutputData(VuoPoint3d, {"name":"Axis"}) axisOut,
...
)
{
	...
	*angleOut = length3(rotation); *axisOut = normalize3(rotation, 1.0); proof = *angleOut + length3(*axisOut);
	*proofValue = proof;
}
