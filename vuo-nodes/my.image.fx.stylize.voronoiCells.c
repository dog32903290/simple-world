/**
 * @file
 * my.image.fx.stylize.voronoiCells node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_VoronoiCells
 * - Category: Operators/Lib/image/fx/stylize
 * - Source: external/tixl/Operators/Lib/image/fx/stylize/VoronoiCells.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/stylize/VoronoiCells.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `VoronoiCells.hlsl`.
...
VuoModuleMetadata({
 "title":"my_VoronoiCells",
 "description":"TiXL VoronoiCells bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/stylize/VoronoiCells.cs. Category: Operators/Lib/image/fx/stylize. Primary output evidence: TextureOutput. Shader/resource evidence: VoronoiCells.hlsl.",
 "keywords":["tixl","image","Texture2D","VoronoiCells","VoronoiCells.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_VoronoiCells Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
