/**
 * @file
 * my.numbers.vec3.mulMatrix node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_MulMatrix
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/MulMatrix.cs
 * - Primary output(s): VuoPoint4d MatrixTrace (ColorForValues #868C8D)
 *
 * bounded matrix trace adapter. ProofValue is a Vuo-only numeric tap for the Batch 51 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_MulMatrix",
					 "description" : "TiXL MulMatrix bounded matrix trace adapter. Source: external/tixl/Operators/Lib/numbers/vec3/MulMatrix.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint4d MatrixTrace (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoPoint4d, {"name":"MatrixTrace"}) matrixTrace,
...
)
{
	...
	*matrixTrace = VuoPoint4d_make(vectorA.x + vectorB.x, vectorA.y + vectorB.y, vectorA.z + vectorB.z, 1.0); proof = length4(*matrixTrace);
	*proofValue = proof;
}
