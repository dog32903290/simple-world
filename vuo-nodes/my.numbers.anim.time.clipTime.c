/**
 * @file
 * my.numbers.anim.time.clipTime node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_ClipTime
 * - Category: Operators/Lib/numbers/anim/time
 * - Source: external/tixl/Operators/Lib/numbers/anim/time/ClipTime.cs
 * - Primary output(s): VuoReal Time (ColorForValues #868C8D)
 *
 * exact scalar clamp adapter. ProofValue is a Vuo-only numeric tap for the Batch 45 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_ClipTime",
					 "description" : "TiXL ClipTime exact scalar clamp adapter. Source: external/tixl/Operators/Lib/numbers/anim/time/ClipTime.cs. Category: Operators/Lib/numbers/anim/time. Primary output(s): VuoReal Time (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*timeOut = clampReal(timeValue, fmin(a, b), fmax(a, b)); proof = *timeOut;
	*proofValue = proof;
}
