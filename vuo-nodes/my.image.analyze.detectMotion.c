/**
 * @file
 * my.image.analyze.detectMotion node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_DetectMotion
 * - Category: Operators/Lib/image/analyze
 * - Source: external/tixl/Operators/Lib/image/analyze/DetectMotion.cs
 * - Default evidence: external/tixl/Operators/Lib/image/analyze/DetectMotion.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `DetectMotion.cs`.
...
VuoModuleMetadata({
 "title":"my_DetectMotion",
 "description":"TiXL DetectMotion bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/analyze/DetectMotion.cs. Category: Operators/Lib/image/analyze. Primary output evidence: TextureOutput. Shader/resource evidence: DetectMotion.cs.",
 "keywords":["tixl","image","Texture2D","DetectMotion","DetectMotion.cs","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_DetectMotion Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
...
(
		...
		VuoInputData(VuoImage, {"name":"VideoTexture"}) videoTexture,
