/**
 * @file
 * my.image.generate.pattern.numberPattern node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_NumberPattern
 * - Category: Operators/Lib/image/generate/pattern
 * - Source: external/tixl/Operators/Lib/image/generate/pattern/NumberPattern.cs
 * - Default: Texture=null, TextColor=(1,1,1,1), LineColor=(1,1,1,1), Highlight=(1,1,1,1), OriginalImage=(1,1,1,1), CellSize=(200,8), CellRange=(1,1), Position=(0,0), Offset=100, ScrollOffset=0, ScrollSpeed=1, HighlightThreshold=0 from NumberPattern.t3
 * - Primary output: Texture2D TextureOutput (ColorForTextures #9F008A)
 *
 * Bounded Vuo shader adapter for external/tixl/Operators/Lib/Assets/shaders/img/{fx or generate}/NumberPattern.hlsl.
...
VuoModuleMetadata({
					 "title" : "my_NumberPattern",
					 "description" : "TiXL NumberPattern bounded Vuo shader adapter. Source: external/tixl/Operators/Lib/image/generate/pattern/NumberPattern.cs. Category: Operators/Lib/image/generate/pattern. Primary output: Texture2D TextureOutput (ColorForTextures #9F008A). Default: Texture=null, TextColor=(1,1,1,1), LineColor=(1,1,1,1), Highlight=(1,1,1,1), OriginalImage=(1,1,1,1), CellSize=(200,8), CellRange=(1,1), Position=(0,0), Offset=100, ScrollOffset=0, ScrollSpeed=1, HighlightThreshold=0.",
					 "keywords" : [ "tixl", "texture2d", "image", "generate", "pattern", "NumberPattern", "NumberPattern.hlsl", "bounded approximation", "ColorForTextures", "#9F008A" ],
					 "version" : "1.0.0",
...
struct nodeInstanceData * nodeInstanceInit(void)
{
	...
	instance->shader = VuoShader_make("my_NumberPattern Shader");
...
)
{
	...
	VuoShader_setUniform_VuoInteger((*instance)->shader, "patternKind", 1);
