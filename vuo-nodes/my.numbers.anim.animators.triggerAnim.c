/**
 * @file
 * my.numbers.anim.animators.triggerAnim node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_TriggerAnim
 * - Category: Operators/Lib/numbers/anim/animators
 * - Source: external/tixl/Operators/Lib/numbers/anim/animators/TriggerAnim.cs
 * - Primary output(s): VuoBoolean HasCompleted, VuoReal Result (ColorForValues #868C8D)
 *
 * bounded trigger envelope adapter. ProofValue is a Vuo-only numeric tap for the Batch 54 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_TriggerAnim",
					 "description" : "TiXL TriggerAnim bounded trigger envelope adapter. Source: external/tixl/Operators/Lib/numbers/anim/animators/TriggerAnim.cs. Category: Operators/Lib/numbers/anim/animators. Primary output(s): VuoBoolean HasCompleted, VuoReal Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
					 "keywords" : [ "tixl", "numbers", "ColorForValues", "#868C8D", "batch54" ],
					 "version" : "1.0.0",
					 "dependencies" : [ ],
				 });
...
(
		...
		VuoOutputData(VuoBoolean, {"name":"HasCompleted"}) hasCompleted,
...
)
{
	...
	VuoReal local = clampReal((timeValue - delay) / fmax(0.0001, duration), 0.0, 1.0); *result = base + amplitude * (trigger ? ease01(local, interpolation) : 0.0); *hasCompleted = local >= 1.0; proof = *result + (*hasCompleted ? 0.25 : 0.0);
	*proofValue = proof;
}
