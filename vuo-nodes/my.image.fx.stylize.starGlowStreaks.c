/**
 * @file
 * my.image.fx.stylize.starGlowStreaks node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_StarGlowStreaks
 * - Category: Operators/Lib/image/fx/stylize
 * - Source: external/tixl/Operators/Lib/image/fx/stylize/StarGlowStreaks.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/stylize/StarGlowStreaks.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `StarGlowStreaks.hlsl`.
...
VuoModuleMetadata({
 "title":"my_StarGlowStreaks",
 "description":"TiXL StarGlowStreaks bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/stylize/StarGlowStreaks.cs. Category: Operators/Lib/image/fx/stylize. Primary output evidence: TextureOutput. Shader/resource evidence: StarGlowStreaks.hlsl.",
 "keywords":["tixl","image","Texture2D","StarGlowStreaks","StarGlowStreaks.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_StarGlowStreaks Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
