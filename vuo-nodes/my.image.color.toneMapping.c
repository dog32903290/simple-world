/**
 * @file
 * my.image.color.toneMapping node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_ToneMapping
 * - Category: Operators/Lib/image/color
 * - Source: external/tixl/Operators/Lib/image/color/ToneMapping.cs
 * - Default: Texture2d=null, Mode=4, Exposure=1, Gamma=2.2, CorrectGamma=false from ToneMapping.t3
 * - Primary output: Texture2D Output (ColorForTextures #9F008A)
 *
 * Bounded Vuo shader adapter for Operators/Lib/Assets/shaders/img/fx/ToneMap.hlsl.
...
VuoModuleMetadata({
					 "title" : "my_ToneMapping",
					 "description" : "TiXL ToneMapping bounded Vuo shader adapter. Source: external/tixl/Operators/Lib/image/color/ToneMapping.cs. Category: Operators/Lib/image/color. Primary output: Texture2D Output (ColorForTextures #9F008A). Default: Texture2d=null, Mode=4, Exposure=1, Gamma=2.2, CorrectGamma=false.",
					 "keywords" : [ "tixl", "texture2d", "image", "color", "ToneMapping", "ToneMap.hlsl", "bounded approximation", "ColorForTextures", "#9F008A" ],
					 "version" : "1.0.0",
...
struct nodeInstanceData * nodeInstanceInit(void)
{
	...
	instance->shader = VuoShader_make("my_ToneMapping Shader");
...
(
		...
		VuoInputData(VuoReal, {"default":4.0}) mode,
...
)
{
	...
	VuoShader_setUniform_VuoInteger((*instance)->shader, "filterKind", 10);
