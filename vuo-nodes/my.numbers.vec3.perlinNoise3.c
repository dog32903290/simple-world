/**
 * @file
 * my.numbers.vec3.perlinNoise3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_PerlinNoise3
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/PerlinNoise3.cs
 * - Primary output(s): VuoPoint3d Result (ColorForValues #868C8D)
 *
 * bounded vector noise adapter. ProofValue is a Vuo-only numeric tap for the Batch 51 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_PerlinNoise3",
					 "description" : "TiXL PerlinNoise3 bounded vector noise adapter. Source: external/tixl/Operators/Lib/numbers/vec3/PerlinNoise3.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = VuoPoint3d_make(noise2(vectorA.x * frequency + phase, vectorA.y, seed), noise2(vectorA.y * frequency + phase, vectorA.z, seed + 11), noise2(vectorA.z * frequency + phase, vectorA.x, seed + 23)); proof = length3(*result);
	*proofValue = proof;
}
