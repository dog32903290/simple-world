/**
 * @file
 * my.image.fx.blur.bloom node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Bloom
 * - Category: Operators/Lib/image/fx/blur
 * - Source: external/tixl/Operators/Lib/image/fx/blur/Bloom.cs
 * - Default: Image=null, Intensity=6, ColorWeights=(0.299,0.587,0.114,1), Threshold=0.5, GlowGradient=white, GainAndBias=(0.5,0.5), MaxLevels=10, Blur=1, Clamp=false from Bloom.t3
 * - Primary output: Texture2D Result (ColorForTextures #9F008A)
 *
 * Bounded Vuo shader adapter for TiXL blur shader evidence including Bloom-BrightpassPS.hlsl.
...
VuoModuleMetadata({
					 "title" : "my_Bloom",
					 "description" : "TiXL Bloom bounded Vuo shader adapter. Source: external/tixl/Operators/Lib/image/fx/blur/Bloom.cs. Category: Operators/Lib/image/fx/blur. Primary output: Texture2D Result (ColorForTextures #9F008A). Default: Image=null, Intensity=6, ColorWeights=(0.299,0.587,0.114,1), Threshold=0.5, GlowGradient=white, GainAndBias=(0.5,0.5), MaxLevels=10, Blur=1, Clamp=false.",
					 "keywords" : [ "tixl", "texture2d", "image", "fx", "blur", "Bloom", "Bloom-BrightpassPS.hlsl", "bounded approximation", "ColorForTextures", "#9F008A" ],
					 "version" : "1.0.0",
...
struct nodeInstanceData * nodeInstanceInit(void)
{
	...
	instance->shader = VuoShader_make("my_Bloom Shader");
...
(
		...
		VuoOutputData(VuoImage, {"name":"Result"}) textureOutput
)
{
