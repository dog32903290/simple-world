/**
 * @file
 * my.numbers.vec2.gridPosition node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_GridPosition
 * - Category: Operators/Lib/numbers/vec2
 * - Source: external/tixl/Operators/Lib/numbers/vec2/GridPosition.cs
 * - Primary output(s): VuoPoint2d Position, VuoPoint2d Size (ColorForValues #868C8D)
 *
 * bounded grid adapter. ProofValue is a Vuo-only numeric tap for the Batch 49 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_GridPosition",
					 "description" : "TiXL GridPosition bounded grid adapter. Source: external/tixl/Operators/Lib/numbers/vec2/GridPosition.cs. Category: Operators/Lib/numbers/vec2. Primary output(s): VuoPoint2d Position, VuoPoint2d Size (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoPoint2d, {"name":"Position"}) position,
...
)
{
	...
	VuoInteger columns = frameCount <= 0 ? 1 : frameCount; VuoInteger row = index / columns; VuoInteger column = index % columns; *size = VuoPoint2d_make(fmax(0.0001, input2.x), fmax(0.0001, input2.y)); *position = VuoPoint2d_make(input1.x + column * size->x, input1.y + row * size->y); proof = length2(*position) + length2(*size);
	*proofValue = proof;
}
