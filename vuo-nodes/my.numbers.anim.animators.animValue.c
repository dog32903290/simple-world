/**
 * @file
 * my.numbers.anim.animators.animValue node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_AnimValue
 * - Category: Operators/Lib/numbers/anim/animators
 * - Source: external/tixl/Operators/Lib/numbers/anim/animators/AnimValue.cs
 * - Primary output(s): VuoReal Result, VuoBoolean WasHit (ColorForValues #868C8D)
 *
 * bounded LFO float adapter. ProofValue is a Vuo-only numeric tap for the Batch 54 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_AnimValue",
					 "description" : "TiXL AnimValue bounded LFO float adapter. Source: external/tixl/Operators/Lib/numbers/anim/animators/AnimValue.cs. Category: Operators/Lib/numbers/anim/animators. Primary output(s): VuoReal Result, VuoBoolean WasHit (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = bias + amplitude * sin((timeValue + phase + offset) * rate * 6.2831853); *wasHit = fabs(*result - bias) < 0.05; proof = *result;
	*proofValue = proof;
}
