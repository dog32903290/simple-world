/**
 * @file
 * my.numbers.vec3.process.springVec3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_SpringVec3
 * - Category: Operators/Lib/numbers/vec3/process
 * - Source: external/tixl/Operators/Lib/numbers/vec3/process/SpringVec3.cs
 * - Primary output(s): VuoPoint3d Result (ColorForValues #868C8D)
 *
 * bounded vector spring adapter. ProofValue is a Vuo-only numeric tap for the Batch 52 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_SpringVec3",
					 "description" : "TiXL SpringVec3 bounded vector spring adapter. Source: external/tixl/Operators/Lib/numbers/vec3/process/SpringVec3.cs. Category: Operators/Lib/numbers/vec3/process. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = mix3(vectorA, vectorB, clampReal(strength / fmax(0.0001, tension), 0.0, 1.0)); proof = length3(*result);
	*proofValue = proof;
}
