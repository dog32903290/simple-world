/**
 * @file
 * my.numbers.anim.time.stopWatch node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_StopWatch
 * - Category: Operators/Lib/numbers/anim/time
 * - Source: external/tixl/Operators/Lib/numbers/anim/time/StopWatch.cs
 * - Primary output(s): VuoReal Delta, VuoReal LastDuration (ColorForValues #868C8D)
 *
 * bounded stateless stopwatch adapter. ProofValue is a Vuo-only numeric tap for the Batch 45 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_StopWatch",
					 "description" : "TiXL StopWatch bounded stateless stopwatch adapter. Source: external/tixl/Operators/Lib/numbers/anim/time/StopWatch.cs. Category: Operators/Lib/numbers/anim/time. Primary output(s): VuoReal Delta, VuoReal LastDuration (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
(
		...
		VuoOutputData(VuoReal, {"name":"Delta"}) delta,
		VuoOutputData(VuoReal, {"name":"LastDuration"}) lastDuration,
		VuoOutputData(VuoReal, {"name":"ProofValue"}) proofValue
)
{
	VuoReal proof = 0.0;
	(void)renderTick; (void)mode; (void)timeMode; (void)freeze; (void)uniqueForChild;
	*delta = trigger ? fmax(0.0, timeValue - a) : 0.0; *lastDuration = fmax(0.0, b); proof = *delta + *lastDuration;
	*proofValue = proof;
}
