/**
 * @file
 * my.numbers.anim.time.convertTime node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_ConvertTime
 * - Category: Operators/Lib/numbers/anim/time
 * - Source: external/tixl/Operators/Lib/numbers/anim/time/ConvertTime.cs
 * - Primary output(s): VuoReal Result (ColorForValues #868C8D)
 *
 * bounded tempo conversion adapter. ProofValue is a Vuo-only numeric tap for the Batch 45 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_ConvertTime",
					 "description" : "TiXL ConvertTime bounded tempo conversion adapter. Source: external/tixl/Operators/Lib/numbers/anim/time/ConvertTime.cs. Category: Operators/Lib/numbers/anim/time. Primary output(s): VuoReal Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = (timeMode == 0) ? timeValue : timeValue * fmax(1.0, speedFactor); proof = *result;
	*proofValue = proof;
}
