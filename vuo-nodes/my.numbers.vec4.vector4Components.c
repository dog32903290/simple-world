/**
 * @file
 * my.numbers.vec4.vector4Components node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_Vector4Components
 * - Category: Operators/Lib/numbers/vec4
 * - Source: external/tixl/Operators/Lib/numbers/vec4/Vector4Components.cs
 * - Primary output(s): VuoReal W, VuoReal X, VuoReal Y, VuoReal Z (ColorForValues #868C8D)
 *
 * exact Vector4 adapter. ProofValue is a Vuo-only numeric tap for the Batch 53 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_Vector4Components",
					 "description" : "TiXL Vector4Components exact Vector4 adapter. Source: external/tixl/Operators/Lib/numbers/vec4/Vector4Components.cs. Category: Operators/Lib/numbers/vec4. Primary output(s): VuoReal W, VuoReal X, VuoReal Y, VuoReal Z (ColorForValues #868C8D). ProofValue is Vuo-only.",
					 "keywords" : [ "tixl", "numbers", "ColorForValues", "#868C8D", "batch53" ],
					 "version" : "1.0.0",
					 "dependencies" : [ ],
				 });
...
(
		...
		VuoOutputData(VuoReal, {"name":"W"}) w,
		VuoOutputData(VuoReal, {"name":"X"}) x,
		VuoOutputData(VuoReal, {"name":"Y"}) y,
		VuoOutputData(VuoReal, {"name":"Z"}) z,
...
)
{
	...
	*w = vector4A.w; *x = vector4A.x; *y = vector4A.y; *z = vector4A.z; proof = fabs(*w) + fabs(*x) + fabs(*y) + fabs(*z);
	*proofValue = proof;
}
