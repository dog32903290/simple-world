/**
 * @file
 * my.image.fx.glitch.glitchDisplace node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_GlitchDisplace
 * - Category: Operators/Lib/image/fx/glitch
 * - Source: external/tixl/Operators/Lib/image/fx/glitch/GlitchDisplace.cs
 * - Default: Image=null, Rows=300, Columns=25, Size=1, Amount=1, Threshold=0, Stretch=(3,0.5), Offset=(0.5,0), Scatter=(0.1,0.1), Colorize=(1,0,0.8917217,1), ColorRatio=0.1, Seed=0, Mode=3 from GlitchDisplace.t3
 * - Primary output: Texture2D Output2 (ColorForTextures #9F008A)
 *
 * Bounded Vuo shader adapter for Operators/Lib/Assets/shaders/points/draw/GlitchDisplace.hlsl.
...
VuoModuleMetadata({
					 "title" : "my_GlitchDisplace",
					 "description" : "TiXL GlitchDisplace bounded Vuo shader adapter. Source: external/tixl/Operators/Lib/image/fx/glitch/GlitchDisplace.cs. Category: Operators/Lib/image/fx/glitch. Primary output: Texture2D Output2 (ColorForTextures #9F008A). Default: Image=null, Rows=300, Columns=25, Size=1, Amount=1, Threshold=0, Stretch=(3,0.5), Offset=(0.5,0), Scatter=(0.1,0.1), Colorize=(1,0,0.8917217,1), ColorRatio=0.1, Seed=0, Mode=3.",
					 "keywords" : [ "tixl", "texture2d", "image", "fx", "glitch", "GlitchDisplace", "GlitchDisplace.hlsl", "bounded approximation", "ColorForTextures", "#9F008A" ],
					 "version" : "1.0.0",
...
struct nodeInstanceData * nodeInstanceInit(void)
{
	...
	instance->shader = VuoShader_make("my_GlitchDisplace Shader");
...
(
		...
		VuoInputData(VuoInteger, {"default":300,"suggestedMin":1,"suggestedMax":600}) rows,
		VuoInputData(VuoInteger, {"default":25,"suggestedMin":1,"suggestedMax":120}) columns,
		VuoInputData(VuoReal, {"default":1.0}) size,
		VuoInputData(VuoReal, {"default":1.0}) amount,
		VuoInputData(VuoReal, {"default":0.0}) threshold,
		VuoInputData(VuoPoint2d, {"default":{"x":3.0,"y":0.5}}) stretch,
		VuoInputData(VuoPoint2d, {"default":{"x":0.5,"y":0.0}}) offset,
		VuoInputData(VuoPoint2d, {"default":{"x":0.1,"y":0.1}}) scatter,
		VuoInputData(VuoColor, {"default":{"r":1.0,"g":0.0,"b":0.8917217,"a":1.0}}) colorize,
		VuoInputData(VuoReal, {"default":0.1}) colorRatio,
		VuoInputData(VuoInteger, {"default":0}) seed,
		VuoInputData(VuoInteger, {"default":3}) mode,

		VuoInputData(VuoPoint2d, {"default":{"x":0.0,"y":0.0}}) resolution,
		VuoOutputData(VuoImage, {"name":"Output2"}) textureOutput
)
{
	...
	VuoShader_setUniform_VuoInteger((*instance)->shader, "rows", rows);
	VuoShader_setUniform_VuoInteger((*instance)->shader, "columns", columns);
	...
	VuoShader_setUniform_VuoPoint2d((*instance)->shader, "scatter", scatter);
	VuoShader_setUniform_VuoColor((*instance)->shader, "colorize", colorize);
	VuoShader_setUniform_VuoReal((*instance)->shader, "colorRatio", colorRatio);
	VuoShader_setUniform_VuoInteger((*instance)->shader, "seed", seed);
