/**
 * @file
 * my.numbers.vec3.transformVec3 node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_TransformVec3
 * - Category: Operators/Lib/numbers/vec3
 * - Source: external/tixl/Operators/Lib/numbers/vec3/TransformVec3.cs
 * - Primary output(s): VuoPoint3d Result (ColorForValues #868C8D)
 *
 * bounded transform adapter. ProofValue is a Vuo-only numeric tap for the Batch 51 proof composition.
 */
...
VuoModuleMetadata({
					 "title" : "my_TransformVec3",
					 "description" : "TiXL TransformVec3 bounded transform adapter. Source: external/tixl/Operators/Lib/numbers/vec3/TransformVec3.cs. Category: Operators/Lib/numbers/vec3. Primary output(s): VuoPoint3d Result (ColorForValues #868C8D). ProofValue is Vuo-only.",
...
)
{
	...
	*result = add3(vectorA, vectorB); proof = length3(*result);
	*proofValue = proof;
}
