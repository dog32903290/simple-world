/**
 * @file
 * my.image.fx.distort.fieldToImage node implementation.
 *
 * TiXL parity contract:
 * - Visible title: my_FieldToImage
 * - Category: Operators/Lib/image/fx/distort
 * - Source: external/tixl/Operators/Lib/image/fx/distort/FieldToImage.cs
 * - Default evidence: external/tixl/Operators/Lib/image/fx/distort/FieldToImage.t3
 * - Primary output evidence: TextureOutput; Texture2D proof color ColorForTextures #9F008A unless noted in acceptance docs.
 *
 * Bounded Vuo body-layer adapter for TiXL evidence `FieldToImageTemplate.hlsl`.
...
VuoModuleMetadata({
 "title":"my_FieldToImage",
 "description":"TiXL FieldToImage bounded Vuo adapter. Source: external/tixl/Operators/Lib/image/fx/distort/FieldToImage.cs. Category: Operators/Lib/image/fx/distort. Primary output evidence: TextureOutput. Shader/resource evidence: FieldToImageTemplate.hlsl.",
 "keywords":["tixl","image","Texture2D","FieldToImage","FieldToImageTemplate.hlsl","bounded approximation","ColorForTextures","#9F008A"],
 "version":"1.0.0",
...
{ struct nodeInstanceData *instance=(struct nodeInstanceData *)malloc(sizeof(struct nodeInstanceData)); VuoRegister(instance, free); instance->shader=VuoShader_make("my_FieldToImage Shader"); VuoShader_addSource(instance->shader,VuoMesh_IndividualTriangles,NULL,NULL,fragmentShaderSource); VuoRetain(instance->shader); return instance; }
