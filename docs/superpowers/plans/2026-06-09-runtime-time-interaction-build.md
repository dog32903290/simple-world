...
- ~~律法 debt：platform→runtime 違律；graph.cpp 600+ 行~~ **已清（2026-06-09 review 後）**：`graph.cpp` 拆出 `graph_selftest.cpp`(`7cd54dc`，616→333 行)；platform→runtime 用 **callback 反轉**修掉(`3adde9c`)——capture 只發 raw block 給回呼，DSP 移到 `app/audio_monitor`，platform 零 runtime include。死碼 `ctx.audioLevel/audioHit` 也清了(`eb9e714`)。
...
4. **【刀4·節點臉 UI 對齊】✓**（2026-06-09 17:xx，柏為親測可動）忠實移植 `AudioReactionUi.DrawChildUi`，把刀2 的單色 `PlotHistogram` 換成 ImDrawList 自繪:**①頻段依偵測視窗染色**（`mix(StatusAnimated, gray, factor)`,factor 由 `|f−windowCenter|/windowEdge − windowWidth/windowEdge` 算 → 視窗內橘、外灰,柏為轉 Inspector 即見偵測頻率範圍移動）②threshold 橫線（`min(thr,1)` 夾頂）+ 底部視窗 bracket ③右側 level 表（高=響度,hit 時橘閃,flash decay 由 WasHit 升緣算=view-local transient,key by node id）④**AccumulatedLevel spinner 弧線**（僅 `Output==4` 畫——該模式 level 無上限會頂滿說謊,弧線是唯一誠實讀數;其他模式隱藏,如 TiXL）。**全程純讀 runtime 資料**（spectrum + node.params + outCache）,零 graph mutation。**律法:抽到新 `ui/node_faces.{h,cpp}`**（每運算子一個自訂臉=TiXL `*Ui.cs` 模式;editor_ui.cpp 497→405 行）;check-arch tripwire 過。**eye-hand 親驗**:拖 Threshold→線移、拖 WindowCenter/Width→值連動、Output=AccumulatedLevel→spinner 現+活音訊頻段染橘。柏為親測確認可動。
- **刀1+2+3+4 ✓ — TiXL AudioReaction 功能 + 節點臉 UI 對齊完成(柏為「一模一樣」目標達成)。**
- **殘留(小/裝飾,非功能):** ①TiXL 節點 UI 的 **peak 疊（背景鬼影 bars）** 未畫（window/threshold/spinner 已於刀4 補齊）;Ctrl+拖節點直接調 window/threshold 未做（柏為在 Inspector 調,非痛點） ②TiXL 參數可接線,我們 pinless(inspector-only) ③`ctx.audioLevel/audioHit` 變 vestigial(AudioReaction 改吃 outCache,可清) ④反應性由組合證(DSP selftest+capture 餵法+say/rms),GUI 內「彈下去頻譜亮」是柏為親測。
- **律法 debt（待追蹤,非本次造成）:** `editor_ui.cpp` 仍 405 行（>400 軟限）,殘餘是 toolbar/canvas/inspector 三職責同檔（pre-existing）;canvas 區在並行編輯中,大拆留作獨立任務,不順手重織。
- 律法:DSP/cook 是 runtime 葉子;capture(platform) **已不 include runtime**——透過 callback 反轉(`3adde9c`)把 DSP 擁有權移到 `app/audio_monitor`，依賴方向乾淨。
...
### ⚠ 律法 debt（2026-06-09 review，**柏為決定**）— ✅ 兩項都已修（柏為「好都修」）：①platform→runtime = 選 (b) callback 反轉 `3adde9c`；②graph.cpp 拆 `7cd54dc`。下方保留原始 review 紀錄。
