...
1b. **version-chasing dirty + per-node cache + Command-always + diamond + LIVE-bump**(決策 6/7，地基核心)：每節點 version/cachedOutput；**eager 一趟後序 walk**（版本傳播+重算同趟，dirty→重算/否則回 cache）；**`isLiveSource` 推導＝driver(Automation) ∨ op 宣告（有狀態/Time/Audio）∨ per-output triggerOverride（照 S3 修正；LiveSource kind 已作廢）**；Command/present 永遠跑（被拉幾次跑幾次，照 C2）；值節點 per-pass 去重靠 version/cache；LIVE source 每 pass bump version；**有狀態 op 帶 FxTime 時間門（原語④）**。**selftest 有牙(count-based；健檢修正：單位=evaluation pass 非幀)**：靜態節點第 2 個 pass 起 cook 0 次、LIVE 每 pass 1 次、改靜態 param→重算 1 次→回 0、**值節點 diamond 同 pass 1 次；Command 節點被拉幾次跑幾次（兩父各拉一次→斷言 =2 不是 =1，照 C2）**、**🪤 漏 bump LIVE version → 卡舊的 RED 變體被抓**、**driver 從 Constant↔Automation toggle → LIVE/STATIC 同步翻（證 binding 與 dirty 不分叉）**、**一趟結果 == TiXL 語意兩趟結果 golden（範圍=純值圖；Command 圖等價不宣稱、照 C1 pull-driven）**。
   ...
   > **slice 2b ✅（resolved-param seam + cookResident 三流 parity + stateful resident state,2026-06-11,commit `df88aa1`）**：承重刀=**resolved-params seam**——cook driver 預解析每節點全部 Float input port（flat:`resolveNodeParams` override→binding→wire→stored→def;resident:`resolveResidentFloatInputs` 走 driver），op 改讀 `cc.params`(cookParam/cookVecN/cookInputParam)、**不再碰任何圖模型**（=TiXL slot 系統解析輸入、op body 不走圖）。順帶修掉全 op 的 wire-blind 參數（7d4b34e 同類病全清）；force 參數隨線解析(cookInputParam 讀 forces input 的上游節點)取代 firstOfType by-type 讀（reuse 必死）、force 沒接=不施力（照 TiXL: force 是 input）。cookResident 補滿三流終端(tex 解析度 pin+displayTex/cmd→RenderTarget executor/preview 合成 1-item chain)+per-path 持久 buffer+stateful state（Impl 資源收斂 string key:resident=path、flat=`#id` 前綴防撞;flat key 隨 swap 死）。**point_graph.cpp 489→343 行債已付**（Impl+registry→`point_graph_internal.h` 私有接縫、cookResident→`point_graph_resident.cpp`）。golden `--selftest-residentparity`（單管線手算值:param 線驅動 22/stateful 跨 cook 32/force 線參數+7/tex 終端 32×32+displayTex/preview 42），`-bug` 有牙。**spec 缺口被牙咬出**：RadialPoints op 讀的 RadiusOffset/StartAngle/Cycles 不在 spec ports（舊直讀 n->params 吃得到、seam 後 falls to def→radialop-bug 不咬）→補進 registry（**append 不 insert**——pin id 是 port-index 制,插中間會讓舊存檔接 Center 的線錯位;v2 schema 改 slotId 制根治）。**獨立 refuter(opus,可執行 repro):1 BROKEN 已修+repro 轉 golden**（`ensureState` 不隨 count 長大重配→ParticleSystem 持久粒子 buffer 過小=GPU 越界寫,production 可達[拖大 Count]→修=grow 時 free+重建[正確性>連續性,鏡像 ensureOut grow 規則],golden `--selftest-statecount` flat+resident 雙腿）；2 析出對齊（Automation driver resident 回 0 vs flat 落 stored constant→已對齊 constant fallback,S3 接曲線時不分岔；vec 分量 inputDefs 手寫漏項→bridge 的 atomicSymbolFromSpec 從 NodeSpec 全量生成=結構性關閉）。
   >
   > **production swap ✅（live app 跑常駐圖,2026-06-11,commits `1e64afe`→`635e1c1`）**：`graph_bridge.{h,cpp}`=`atomicSymbolFromSpec`+`libFromGraph`(flat Graph→SymbolLibrary;child id==node id→resident path==id 字串→**rebuild 跨次 per-path GPU state 存活**;兼批次 2 舊檔 migration 載入器,非丟棄碼)。`ResidentNode::extOut[3]`+evalResidentFloat no-evaluate 路=flat outCache 鏡像（AudioReaction 外部 cooker 寫入;S1 收編再精煉權威）。`app/frame_cook.{h,cpp}`（app 區,main 回 333 行守殼律）=每幀:鏡像 rebuild-on-revision（document.h **鏡像契約:任何 g_graph 變動必 bump graphRevision**——CommandStack push/undo/redo+doOpen/doNew+Inspector 兩處 live-drag）→AudioReaction cook（outCache+extOut）→`cookResident(viewTarget 字串)`。**flat cook() production 不再跑**、留作 golden 參照側。golden `--selftest-graphbridge`（真 ops+metallib:預設圖+Const→Radius 線+AudioReaction.Level→Speed 線,3 幀 flat vs resident **逐位元相同**含 stateful GPU sim;-bug 拔線分岔）。⚠ **eye 活體 smoke 未跑**：柏為螢幕睡眠（pmset log 11:31 Display off）→MTKView 無 display link、app 不 tick=環境非碼;螢幕醒後補（graphbridge 已釘 cook 路徑)。**延後(named)**：命令層 patch*/patchLib* 增量取代 rebuild（語意已由 patch golden 釘死）、cookResident 接 pullResidentFloat 值 cache、S1 SourceRegistry 收編、defaultDrawTarget/viewTarget 仍讀 flat（殼層暫許）。
2. ✅ **存檔 v2 + migration（2026-06-11,commits `f847c65`→`dd8badb`）**：`runtime/compound_save.{h,cpp}`=v2 `{formatVersion:2,rootSymbolId,symbols[]}`——**只序列化 compound**（atomic 住 binary registry、以固定 UUID 表引用=決策 4,loader 用 atomicSymbolFromSpec 再生→定義永不與 kernel 漂移=TiXL「operator 住 code、.t3 引 Guid」同構）;compound 自描述（S16:defs 含 id/name/dataType/def,**陣列序=定義序**）;連線陣列序=multi-input 序（保序釘住）;**兩階段 load**;**S15 容錯落地**（缺 symbol→丟 child+警告、dangling wire→丟+警告[下次存檔自癒]、新 formatVersion→警告續讀;「malformed→整檔 false」已死,僅 JSON 不可解析才整檔敗）;v1（無 formatVersion）自動經 libFromGraph migration。app 層:doSave 寫 v2、doOpen 雙格式+修復警告上 status（英文,imgui 無 CJK）;graphFromLib 逆橋（過渡腿,root 含 compound 誠實拒開,批次 3 編輯器 lib-native 後死）。golden `--selftest-savev2`(roundtrip 位元穩+求值等同 15==15/reuse override 存活/atomic 再生/v1 migration+flat↔lib↔flat 位元等同/S15 竄改檔精準局部丟棄);`-bug` 有牙。**獨立 refuter(opus):4 BROKEN 全修+repro 轉 golden 腿**——①NaN/inf override 寫出 `nan` token=自己寫的檔讀不回（整檔敗,違 S15）→writer 永不輸出非有限值(nanClamp 腿)②`sw-type:` fallback 命名空間劫持同名 compound→loader compound 先解析(nsNoHijack 腿)③④逆橋位元等同非真不變式（v2 不存 conn id=v1 artifact,golden 原自我滿足）→契約改誠實=語意 roundtrip、id 正規化釘為預期(oddIdSemantic 腿)+重複 symbol id 兩 phase first-wins+警告。**⚠具名風險(未修,批次 4 動工前提)**：crude_json parse 對非 ASCII byte assert→abort——**柏為幫 compound 取中文名=寫得出讀不回**,combine 出貨前必解(寫端 \u 跳脫/換 parser/名稱消毒)。**延後(named):animator 段(S3)/CompositionSettings(S18)/children isBypassed+outputs/multiInputIndex——功能未生,schema 自然擴。**
