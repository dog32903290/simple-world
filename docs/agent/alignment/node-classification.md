...
- **驗證證據**:上一棒標 partial，但三項視覺特徵在 palette 中全缺：quick_add.cpp:225 `ImGui::Selectable(displayName(id).c_str(), selected, ...)` = 純文字 Selectable，只有 BeginDisabled(cyclic) 灰化 (line 224)。無型別色、無 dataType 後綴、無 hover tooltip。校正上一棒的『需建 dataType→color 表』：該表已存在 — node_style.cpp:65 `ImU32 typeColor(const std::string& dataType)`（TiXL UiColors 港），且 node_draw.cpp:66 已用『first output dataType』替節點上色，pattern 現成。description 欄位仍缺（grep description/help/summary/tooltip 於 graph.h/compound_graph.h = 零）。標 real-gap（palette 三特徵全無），但成本低於 spec 想像（顏色表+first-output pattern 可直接重用）。
