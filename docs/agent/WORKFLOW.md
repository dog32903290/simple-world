...
> **⚠2026-07-04 §一 分層表 STALE 警示**：下面 §一 的「角色×模型」表是 2026-06-13 **Fable 不可用、Opus
> 當家**那個時代寫的，預設 orchestrator=Opus。**Fable 5 已回歸且 capability 在 Opus 4.8 之上**（Mythos-tier）。
> 新分層原則（待柏為拍板 fable 角色後正式改表）：**最聰明的腦擺最硬的單點判斷**（fork 裁決／假綠燈獵殺／
> 最難 triage＝刀刃 → Fable）；**長又雜的調度**（盯工人／記帳／合流＝刀背）用便宜腦；**判斷閘永遠獨立於
> 生產者**（不因誰聰明而省，越聰明的假綠燈越有說服力）。**韌性**：orchestrator 角色不綁死 Fable（它間歇，
> 2026-06-13 掉線 Opus 接手的歷史＝單點故障教訓）。正式改表前，§一按舊表讀但知道這個方向。
