...
> **★★ 2026-06-27 四次 GROUND-TRUTH（value-emit pass BUILT，write-back）：上行的 ✦point-into-frame value-emit pass = ✅BUILT @b77789e**（fe35682，--bite 508；named defer `defer-pointtomatrix-needs-point-into-frame-pass` RESOLVED）。機制：additive 第二 pass `cookPointValueOutputNodes`（resident_point_value_output_cook.cpp）在 `pg.cookResident` 之後跑（frame_cook.cpp:391），host-read 完成的 StorageModeShared point buffer（`PointGraph::residentCookedPoints(path)`），重用 golden `pointToMatrixRows`；**零 point_graph.cpp / EvaluationContext / resident-recursion 改動（純 additive）**。已 WIRE：**PointToMatrix（emit 半邊,evaluate==nullptr 已翻正）+ GetPointDataFromList @b77789e**。剩同形 fan-out 葉（unblocked-but-not-yet-ported）：GetTextureSize（texture-source 兄弟,DIFFERENT accessor=texture 非 point）/ point-attribute / SamplePointAtList readers。**★兩個 NAMED LATENT RISK（今日 inert=0 consumer,柏為蓋 consumer rail 前必解，詳 SEAM_COMPLETION_PLAN §0′-latent-risks）：`latent-pointvalue-emit-one-frame-late`（pass 在 cookResident 之後寫,in-graph consumer 讀上一幀；sw forward push-cook 本質,golden 用 stub 未覆蓋 multi-frame）/ `latent-stale-points-off-display-subtree`（outBuf 無 per-frame invalidation + target-driven cook,Points src 在顯示 subtree 之外→讀 STALE 前一幀 buffer 非 identity）。NAMED FORK `fork-getpointdata-vec-as-scalar-ports`（Vec3/float/Vec4 攤到 scalar extOut[8] 3+1+4,值相等,同 RequestedResolution Size-as-2-floats fork）。**
