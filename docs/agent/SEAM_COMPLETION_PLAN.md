...
>
> **★★ 2026-06-27 final-autonomous-seam scout（code-cited，再校正下表「承重」=「rail 已建，只剩葉」的三條）：mesh-input / CPU-readback / StructuredList 三條 rail 全 BUILT（非待蓋承重 seam）。三條合計的 autonomous leaf 恰 3 顆（FindClosestPointsOnMesh / SampleCpuPoints / JoinLists）現已 ✅全 BUILT @504f149（--bite 506，refuter MERGE-SAFE，autonomous leaf residue 觸底），其餘全是柏為域（render / sim / value-emit / ICamera）。誠實殘渣小。fork 記錄：joinlists-length-deferred（Length int→value rail，deferred 柏為域）/ samplecpupoints-singleinput / cpupoint-reuses-swpoint。**
>
...
> | **mesh-input rail = ✅BUILT**（PointCookCtx::meshVtx/meshIdx，point_graph_cook_ctx.h:88，filled point_graph.cpp:355-356；MeshVerticesToPoints/PointsOnMesh 已騎，是葉非 seam-gated） | autonomous leaf **FindClosestPointsOnMesh ✅BUILT @504f149**（brute-force tri-distance .hlsl，無 BVH/camera；point_ops_findclosestpointsonmesh.cpp）。**DrawMeshAtPoints2 = Slot<Command> render（柏為 render tail，非此 rail）；SimFollowMeshSurface = PointSimulation-gated（柏為 E-hard）** | rail 已建·葉 ✅BUILT |
> | **CPU-readback rail = ✅BUILT**（production：upstream op commit+wait，SwPoint bag StorageModeShared，ops 直讀 contents()；ReadPointColors=colorlist_ops_readpointcolors.cpp + PointsToCPU=pointlist_ops_pointstocpu.cpp **已註冊/已建**） | autonomous leaf **SampleCpuPoints ✅BUILT @504f149**（PointList→PointList，host Bezier+quat；pointlist_ops_samplecpupoints.cpp；fork=samplecpupoints-singleinput / cpupoint-reuses-swpoint）。**PointToMatrix-emit ✅BUILT @b77789e（value-emit pass 已建,evaluate==nullptr 已翻正）；CpuPointToCamera（ICamera/projection object）仍 = 柏為域** | rail 已建·葉 ✅BUILT |
> | **StructuredList rail = ✅BUILT**（producer = 已建 PointList 第 7 flow（host vector<SwPoint>）；cook driver 已展開 MultiInput PointList port，point_graph_hostvalue_cook.cpp:64-108。非 device-IO / 非 dict-gated） | autonomous leaf **JoinLists ✅BUILT @504f149**（MultiInput concat → Result，Result-only；pointlist_ops_joinlists.cpp；fork=joinlists-length-deferred=其 int `Length` 第 2 output 騎 value rail = deferred 柏為域）。**GetPointDataFromList ✅BUILT @b77789e（point-into-frame value-emit pass 已建,output Vector3/float/Vector4 到 value rail；fork=fork-getpointdata-vec-as-scalar-ports=Vec3/float/Vec4 攤到 scalar extOut[8] 3+1+4,值相等）** | rail 已建·葉 ✅BUILT |
...
> | **★point-into-frame value-emit pass = ✅BUILT @b77789e**（fe35682，--bite 508；named defer `defer-pointtomatrix-needs-point-into-frame-pass` RESOLVED） | ✅WIRE：PointToMatrix（emit 半邊,從 evaluate==nullptr 翻正）+ GetPointDataFromList @b77789e。剩同形 fan-out 葉（unblocked-but-not-yet-ported）：GetTextureSize（texture-source 兄弟,DIFFERENT accessor=texture 非 point）/ point-attribute / SamplePointAtList readers | **機制**：additive 第二 pass `cookPointValueOutputNodes`（resident_point_value_output_cook.cpp）在 `pg.cookResident` 之後跑（frame_cook.cpp:391），host-read 完成的 StorageModeShared point buffer（`PointGraph::residentCookedPoints(path)`,resident-path keyed），重用 golden `pointToMatrixRows`。**零 point_graph.cpp / EvaluationContext / resident-recursion 改動（純 additive）**。詳見下「§0′-latent-risks」 |
...
>
> **柏為域分界（乾淨線）= 凡「從 point buffer emit 到 value rail / 是 camera / render / simulate」皆柏為域**（measured 量級）：camera3d/3D-render tail ~40-60；PointSimulation pool ABI ~15-25；value-emit-from-buffer seam ~10-15（一塊 seam）；device-IO（dict-ctx deferred = device-IO，loaders，audio/MIDI/file）~30-40；gizmo/loader/field-SDF tail/mesh-gen tail ~80-100（measured 近零 autonomous yield：field-SDF 1/18、mesh-gen 0/15）。**dict-ctx 不是乾淨 host rail，死在 device-IO producer → 柏為域非 autonomous；C cook-core sub-seam String ctx-var ✅BUILT@b03cddd（Get/SetStringVar on typed `stringVars` channel — 結果 IS Vec3 鏡像，TiXL StringVariables 是 typed dict 非 boxed-object；writer-first 2-pass string cook + per-frame `stringVars.clear()`，.t3 VariableName="s"/FallbackDefault=""；named defer `defer-setstringvar-subgraph-command-rail` = SubGraph push/restore 待 SetStringVarCmd Command type；named fork `fork-setstringvar-echo-output` = TiXL Slot<Command> Output→String echo）；剩 Command ctx-var sub-seam（Command/Execute rail）仍 medium seam，非 Vec3 鏡像。**
>
> #### §0′-latent-risks：point-into-frame value-emit pass 的兩個 NAMED LATENT RISK（★柏為蓋 consumer rail 前必讀）
>
> 此 pass @b77789e 今日 **inert（0 forward consumer）**——PointToMatrix/GetPointDataFromList 的 emit 半邊已 wire,但沒有任何 in-graph 節點在消費這些 emit 值。下面兩個 risk 在 inert 狀態無害,**但柏為蓋 PointToMatrix→camera / 任何 in-graph value-from-point consumer rail 前必須先解**：
>
> - **`latent-pointvalue-emit-one-frame-late`**：新 pass 在 `cookResident` **之後**寫 extOut/extColorOut,而其餘所有 emit pass 都在 `cookResident` **之前**寫。未來 in-graph consumer 在 cookResident 期間拉這些值 → 讀到「**上一幀**」的值。這是 sw forward push-cook 的本質（TiXL 是 pull-graph,無此延遲）。goldens 用 stub accessor,**未覆蓋** multi-frame consumer 路徑。
> - **`latent-stale-points-off-display-subtree`**：`PointGraph::outBuf` 無 per-frame invalidation + resident cook 是 target-driven（只有顯示節點的 upstream subtree 會 cook）。PointToMatrix/GetPointData 的 Points src 若**在顯示 subtree 之外** → 讀到 STALE 前一幀 buffer（non-null,錯 count）→ emit stale points,非 identity。
>
> NAMED FORK：**`fork-getpointdata-vec-as-scalar-ports`**=Vec3/float/Vec4 攤到 scalar extOut[8]（3+1+4）,值相等,同 RequestedResolution 的 Size-as-2-floats 已立 fork。
>
> 同夜其他 debunked：keyframe-anim BUILT（騎 curve_animator）、matrix output BUILT（resident_matrix_output_cook.cpp；★emit 半邊現由 value-emit pass 接上 @b77789e）、GPU-compute generators parity-complete、dict-ctx 是 pure-host 非 device-IO、string-wire 閘（task_32b5b6e5）已 CLOSED、compound-graph-host = 假縫 debunked（5ccc637）。
...
|------|-------|------|------|------|
| **string-value-rail** ✅ | ~34 | R1 | 弱 | **✅ 補完 commit `b247602`（2026-06-20，7 agent 承重戰）。** string host-channel rail（第六條 cook flow 鏡像 FloatList，非 evalString 避核心風險）。3 葉子 StringLength/FloatToString/CombineStrings。★FloatToString C# 格式化深坑挖到 .NET 版本 ground-truth（TiXL net10.0→F8 暴露 IEEE-754 noise，非 .NET Framework 補零）。fork-6: StringLength.length 存 floatListBuf,下游橋延後 list-routing。實際解鎖待 Phase C 採(Layer A ~13 + Layer B StringList ~7)。**✅ P0 結構閘已關（2026-06-22）：resident string-wire seam commit `0bb25e2`，task_32b5b6e5 CLOSED。** extStrOut channel on ResidentNode（鏡像 extColorOut），R-2 dual-leg（LEG 20 CombineStrings wire-order / LEG 21 StringLength wired vs const），refuter MERGE-SAFE。**★STRING PHASE C NOW OPEN：~34 B2 ops 可採，R-2 flat+resident golden 各。** 見 DEBT_LEDGER §C（task_32b5b6e5 closed）/ Cut 102。|
