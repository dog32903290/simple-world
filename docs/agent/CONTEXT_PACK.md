...
# B 軌 UI/skin parity（非節點；派 UI/體驗 lane 第零步——選還活的 gap）
tools/ui_census.sh --gaps                # 現算缺口；DONE=快照已 stale 別重做
# ↑ 每條 gap 的對齊規格+TiXL 源碼依據+sw 行號 → docs/agent/alignment/<area>.md
```
