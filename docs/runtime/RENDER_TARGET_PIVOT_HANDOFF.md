...
| **batch 2** | **`cook()` 終端調度接上三流 + DrawPoints 改 cmd op + RenderTarget 接執行者** | **✓（全 selftest 綠）** |
| **batch 3** | **RenderTarget NodeSpec + `Command`/`Texture2D` port 型別 + DrawPoints Command out + Inspector 解析度 enum + ClearColor + RenderTarget 自有解析度貼圖** | **✓ 本 session（柏為親手 GUI 驗證：加節點→接線→Output 黑轉亮）** |
| batch 3.5 | DrawPoints 視覺參數家族（per-point Color / Blend / PointSize → RenderDrawItem 帶過去）| ⬜（draw 招牌打磨，可選，非完成定義） |
...
## batch 3 落地（2026-06-10）

**完成定義達成（柏為親手 GUI 驗證，eye/hand harness）**：Add Node → RenderTarget（資料驅動選單自動出現）→ 節點畫出正確 port（command 青=Command、out 洋紅=Texture2D #9F008A）+ Inspector 顯示 Resolution 下拉/CustomW/H/ClearColor → 拖 DrawPoints.out(Command)→RenderTarget.command → **Output 由黑轉亮（clean.png nonBlack 0→2552，粒子雲渲進 RenderTarget 512² WindowFollow 貼圖）**，59.9 FPS 零崩。

實作：① 新 port dataType `Command`/`Texture2D`（`node_style.cpp` 配色、`output_window.cpp` Texture2D 可預覽）② DrawPoints += Command out port、新 RenderTarget NodeSpec（Command in/Texture2D out/Resolution enum/CustomW/H/ClearColor vec4）③ `PointGraph` 每節點自有貼圖 `ensureTex`（依 Resolution 重配、RESOURCE_LIFETIME）+ `displayTex`（tex 終端顯示自有解析度貼圖，cmd/preview 仍用 window-sized target）④ cook() tex 終端 `resolveRenderResolution`→`ensureTex`→執行 ⑤ cookRenderTarget 讀 ClearColor 參數。golden `rendertargetwired`（真三流 wired，term/size/lit 三斷言；injectBug 斷線→黑→FAIL）。**過渡不變式不變**（legacy draw 仍活到 batch 4）。
...
batch 3 完成定義已達成（見上）。剩下都是可選打磨/收尾，柏為開批：
- **batch 3.5**：DrawPoints 視覺參數家族（per-point Color / Blend mode / PointSize）。`RenderDrawItem` 已留 viewExtent；加 color/blend/size 欄位 + DrawPoints NodeSpec 參數 + cookDrawPoints 帶過去 + RenderTarget 執行者套用。
- **batch 4**：退役 legacy draw（`PointDrawFn`/`drawReg`）——把 9 個葉測的 `registerDrawOp` capture shim 遷成 cmd op capture（`stubDrawCapture` 範本），刪 cook()/defaultDrawTarget 的 legacy 分支。一次清掉過渡碼。
- **可選**：RenderTarget 的 Multisampling / WithDepthBuffer / TextureFormat（TiXL 合約有，目前略）；Texture2D 下游消費者（image filter / feedback）才需要時再開。
- **已知 debt**：cookRenderTarget 每幀建 PSO（live perf，要時快取）；ClearColor alpha 目前進 clear 但 target 是 opaque 顯示。
